"""Train anti-shortcut CT-DRR/MRCP-sum common feature maps on SXH synthetic pairs."""
...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "runs" / "feature_common_comir_antishortcut_sum"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SXH anti-shortcut CoMIR feature maps with MRCP sum projections")
...
def save_debug_images(output_dir: Path, step: int, raw, canonical_xray, canonical_mrcp, augmented, canonical_bile) -> None:
    ...
    write_gray_png(debug_dir / f"{stem}_raw_mrcp_sum.png", raw.mrcp_projection[0, 0].detach().cpu().numpy())
    write_gray_png(debug_dir / f"{stem}_canonical_ct_drr.png", canonical_xray[0, 0].detach().cpu().numpy())
    write_gray_png(debug_dir / f"{stem}_canonical_mrcp_sum.png", canonical_mrcp[0, 0].detach().cpu().numpy())
    write_overlay_png(
    ...
    write_gray_png(debug_dir / f"{stem}_augmented_mrcp_sum.png", augmented.mrcp[0, 0].detach().cpu().numpy())
...
def main() -> None:
    ...
    renderer = SXHFeatureTrainingRenderer(projection_mode="sum", render_chunk_size=args.render_chunk_size, device=args.device)
    ...
    print(f"completed {args.steps} anti-shortcut CoMIR sum-projection steps; output: {output_dir}")
