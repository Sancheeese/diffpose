...
from ours.utils.CT_dataset_PA import IntubationDataset, create_circle_mask, create_circle_mask_reverse, toZeroOne
import kornia
import cv2
...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 13], [0.5, 0.5], device=self.device, step=[None, 1])
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 8], [0.5, 0.5], device=self.device, step=[None, 8 // 2])
        ...
        # self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 32], [0.5, 0.5], device=self.device, step=[None, 16])
        ...
        self.transforms_119 = Transforms(self.drr.detector.height, radius=119)
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            loss = weight * dice + (1 - weight) * gradient_ncc(pred_img, self.img, self.total_mask) + weight * self.criterion(pred_img, self.img)            # ssim = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * self.total_mask, toZeroOne(self.img) * self.total_mask, window_size=11, reduction='mean')
    ...
    def run(self, idx):
        ...
        pred_img = self.transforms_119(img_ori, reverse=False)
        gt_img = self.transforms_119(gt_img, reverse=False)
        err = toZeroOne(pred_img) - toZeroOne(gt_img)
        ...
        plt.axis('off')
        plt.savefig(f"zyl_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        # optimizer2, scheduler2 = self.initialize_optimizer(registration, r_lr=10e-3, t_lr=5)
        ...
        for i in itr:
            ...
            # overlay_grayscale_with_red_tensor(img, edge, alpha=0.2)
            ...
            # # pred_edge = get_edge(pred_img, white_mask)
            # pred_edge = torch.clamp(toZeroOne(pred_edge), max=0.8)
            # # pred_edge += 0.2
            # pred_edge = toZeroOne(pred_edge)
            # pred_edge += 0.1

            # print(wei_pred.min())
            # print(wei_pred.max())
            # plt.figure()
            # plt.imshow(wei_pred.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
            # plt.show()
        ...
        plt.axis('off')
        plt.savefig("pred_img.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        edge = get_edge(pred_img_bone, circle_mask)
        overlay = overlay_grayscale_with_red_tensor_save2(img_ori, edge, "./overlay.png", alpha=1)
        overlay = torch.tensor(overlay)
        pred_img = self.transforms_119(pred_img, reverse=False)
        gt_img = self.transforms_119(gt_img, reverse=False)
        err = toZeroOne(pred_img) - toZeroOne(gt_img)
        # err[err < 0] = toZeroOne(err[err < 0]) - 1
        # err[err > 0] = toZeroOne(err[err > 0])
        ...
        plt.imshow(err.detach().cpu().squeeze(), cmap="bwr", norm=norm)
        plt.axis('off')
        plt.savefig(f"zyl_diff_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        save_err_with_black(pred_img, gt_img, f"zyl_diff_err_black_{idx}.png", 119)
        save_err_half(pred_img, gt_img, img_ori, overlay, edge, "./zyl_diff", idx)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/zyl_800_norm_pa_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/zyl_800_norm_pa_best2.ckpt", map_location="cuda:1")
    ...
    # specimen = IntubationDataset(root, x_root, z_offset=100, z_cut=600, factors=[3, 0.5, 0.5])
    ...
    nccs = []
    for idx in tqdm(range(44, len(specimen)), ncols=100):
        df, ncc = registration.run(idx)
        df.to_csv(
            f"runs/no_stage/zyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
