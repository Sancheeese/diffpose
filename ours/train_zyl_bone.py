...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            weight_map = torch.tensor(img_bone).to(torch.float32)
            weight_map = transforms(weight_map, reverse=False)
            weight_map = (weight_map - weight_map.min()) / (weight_map.max() - weight_map.min())
            weight_map[weight_map >= 0.01] = 1
            weight_map[weight_map < 0.01] = 1
            weight_map = add_circle_mask(weight_map)
            for im in weight_map:
                plt.figure()
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_800_norm_bone_best.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_800_norm_bone_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_800_norm_bone_best.ckpt")
            break
