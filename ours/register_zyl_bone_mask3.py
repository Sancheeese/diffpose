...
from ours.utils.grad_similar import gradient_ncc, get_edge
...
class Registration:
    def __init__(
        ...
        # self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_5_white/50_net_G.pth",
        ...
        # self.style_change =  StyleChanger("/media/sda1/PersonalFiles/yx/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_5_white3/80_net_G.pth",
        ...
        # self.style_change =  StyleChanger("cut/ckpt/7P0_net_G.pth",
    ...
    def initialize_registration(self, img, guide_mask=None):
        with torch.no_grad():
            offset, mask_loss = self.model(img, None, None, guide_mask)
            features = None
    ...
    def run(self, idx):
        ...
        registration = self.initialize_registration(img, guide_mask=tube)
        # img = img_ori
        optimizer, scheduler = self.initialize_optimizer(registration, r_lr=15e-3, t_lr=7.5)
        ...
        for i in itr:
            ...
            # pred_img_bone = self.drr_bone(None, None, None, pose=gt_pose, bone_attenuation_multiplier=3)
            ...
            # pred_edge += 0.1
            #
            # wei_pred = torch.zeros_like(pred_img)
            # wei_pred[circle_mask > 0] = torch.exp(-a * toZeroOne(pred_img[circle_mask > 0]))
            # wei_pred[circle_mask <= 0] = wei_pred[circle_mask > 0].min()
            # print(wei_pred.min())
            # print(wei_pred.max())
            # plt.figure()
            ...
            # l1 = (1 - weight) * gradient_ncc(pred_img, img, total_mask)
            # l2 = weight * self.criterion2(pred_img, img, None, wei_img)
            # loss = (1 - weight) * gradient_ncc(pred_img, img, total_mask) + weight * self.criterion2(pred_img, img, None, wei_img)
            loss = (1 - weight) * gradient_ncc(pred_img, img, total_mask) + weight * self.criterion(pred_img, img)
        ...
        # # rot = np.array([1.8297545909881592, -0.7949684858322144, -0.5890845656394958])
        # # xyz = np.array([255.49586486816406, 6.26397705078125, 110.58015441894531])
        ...
        # t_t_l = 20
        # # [顺逆时针， ， ]
        # s = [t_r_l, t_r_l, t_r_l, t_t_l, t_t_l, t_t_l]
        # option = {
        #     # "seed": 123,
        #     # "CMA_rankmu": 0.2,
        #     "popsize": 15,
        ...
        # r = x[:3].unsqueeze(0)
        # t = x[3:].unsqueeze(0)
        # # tensor([[ 1.8486, -0.7842, -0.5625]], device='cuda:0')
        # # tensor([[258.5076,  10.1461, 112.1029]], device='cuda:0')
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/no_stage/zyl_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/zyl_800_norm_bone_mask3_best_500.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/zyl_800_norm_bone_mask5_best.ckpt", map_location="cuda:1")
    ...
    nccs = []
    for idx in tqdm(range(14, len(specimen)), ncols=100):
