...
from utils.registration_sparse import PoseRegressor, SparseRegistration, VectorizedNormalizedCrossCorrelation2d
...
class Registration:
    def __init__(
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 100, 20, 5], [0.25, 0.25, 0.25, 0.25], device=self.device)
        ...
        self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_nec1/50_net_G.pth",
                       device=self.device,
                       resize=256)
    ...
    def run(self, idx):
        ...
        sparseNCC = VectorizedNormalizedCrossCorrelation2d()
        for _ in itr:
            t0 = time.perf_counter()
            optimizer.zero_grad()
            pred_img, mask, img_print, circle_mask = registration(100, 13)
            # loss = self.criterion(pred_img, img)
            loss = sparseNCC(img, pred_img, mask, circle_mask, 100, 13, reverse=True)
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            img_save = img_print.detach().clone()
            img_save = img_save.cpu().squeeze(0).squeeze(0)
            img_save = np.array(((img_save - img_save.min()) / (img_save.max() - img_save.min())) * 255).astype(np.uint8)
