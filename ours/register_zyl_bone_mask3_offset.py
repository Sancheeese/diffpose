...
class Registration:
    def __init__(
        ...
        # self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 64, 8], [0.3, 0.4, 0.3], device=self.device, step=[None, 32, 4])
        ...
        self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_5_new3/100_net_G.pth",
        # self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_5_new2/70_net_G.pth",
    ...
    def run(self, idx):
        ...
        rot_data = offset.get_rotation(parameterization="so3_log_map")
        xyz_data = offset.get_translation()
        ...
        for i in range(self.n_iters):
            ...
            # pose = self.specimen.isocenter_pose.compose(back_pose).compose(pose).compose(center_pose)
            ...
            # 使用对应阶段的损失函数
            grad_loss = gradient_ncc(pred_img, img, total_mask)
            ncc = self.criterion2(pred_img, img)
            # loss = ncc
            # loss = grad_loss
            loss = (1 - weight) * grad_loss + weight * ncc
            loss.backward()
            optimizer.step()
            scheduler.step()
            ...
            if i == self.n_iters - 1:
                l_gt = (1 - weight) * gradient_ncc(gt_img, img, total_mask) + weight * self.criterion2(gt_img, img)
                l_gt2 = self.criterion2(gt_img, img)
                mid = pred_img
                plt.figure()
                plt.imshow(img_save, cmap='gray')
                plt.show()
...
def main(id_number, parameterization):
    ...
    nccs = []
    for idx in tqdm(range(57, len(specimen)), ncols=100):
