...
from ours.utils.model2 import PoseRegressor, PoseRegressorCat, PoseRegressorAttnWei, PoseRegressorAttnNoWei, \
    PoseRegressorCoe, PoseRegressorMultiSp, PoseRegressorNoAttn
...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            pred_offset = model(img, img_bone_tube)
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_noattn_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/zyl_noattn_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/zyl_noattn_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    model = PoseRegressorNoAttn(**model_params)
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_noattn_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_noattn_last.ckpt")
