...
from ours.Bipose.model import CSPConvNeXt, ConvNeXt
...
from ours.utils.model2 import PoseRegressor, PoseRegressorCat
...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            loss = 1 - ncc + 1e-2 * double_geodesic + 30 * mse
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_cat_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/zyl_cat_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/zyl_cat_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    model = PoseRegressorCat(**model_params)
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_cat_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    main(batch_size=4, n_batches_per_epoch=200, n_epochs=1000, accumulate_step=1, lr=0.001)
    while True:
        try:
            ...
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_cat_last.ckpt")
