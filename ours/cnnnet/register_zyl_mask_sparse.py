...
from ours.utils.img_utils import flip_img_w, overlay_grayscale_with_red_tensor, overlay_grayscale_with_red_tensor_save, \
    canny_edge_torch, err_half_lr, err_half_ud, overlay_grayscale_with_red_tensor_save2, err_half_4, save_err_half, \
    save_err_with_black
...
class Registration:
    ...
    def run(self, idx):
        ...
        canny_edge_torch(toZeroOne(img))
        ...
        err = toZeroOne(gt_img) - toZeroOne(gt_img)
        print(err.min())
        print(err.max())
        ...
        plt.axis('off')
        plt.savefig(f"zyl_err_gt_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay = overlay_grayscale_with_red_tensor_save2(img_ori, edge, f"./ret/zyl_overlay_{idx}.png", alpha=1)
        overlay = torch.tensor(overlay)
        circle_mask = create_circle_mask(256, 119).to(self.device).unsqueeze(0).unsqueeze(0)
        ...
        plt.show()
        save_err_with_black(gt_img, gt_img, f"err_black.png", 119)
        save_err_half(pred_img, gt_img, img_ori, overlay, edge, "./ret/zyl", idx)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/zyl_coespdeco_best.ckpt", map_location="cuda:1")
    ...
    specimen = IntubationDataset(root, x_root, y_offset=300, z_cut=650, factors=[1, 0.2, 0.2])
    ...
    # for idx in tqdm(range(44, 88), ncols=100):
    # for idx in tqdm(range(2, 30), ncols=100):
        df, ncc = registration.run(idx)
        df.to_csv(
            # f"runs/mask/zyl/localncczyl_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/zyl/low_zyl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/zyl/zyl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/zyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
