...
from ours.utils.model2 import PoseRegressor, PoseRegressorCat, PoseRegressorAttnWei, PoseRegressorAttnNoWei, \
    PoseRegressorCoe, PoseRegressorCoeDeco, PoseRegressorCoeSpDeco, PoseRegressorCoeSpDeco2, PoseRegressorGSpDeco
...
def load(height, device):
    ...
    specimen = IntubationDataset(root, x_root, y_offset=300, z_cut=650, factors=[2, 0.5, 0.5])
    ...
    for epoch in range(n_epochs + 1):
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_coespdeco2_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/zyl_coespdeco2_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/zyl_coespdeco2_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_coespdeco2_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    # main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_coespdeco2_epoch300.ckpt")
    while True:
        try:
            # main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_800_norm_bone_mask3_pa_best.ckpt")
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_coespdeco2_last.ckpt")
            # main(batch_size=4, n_batches_per_epoch=200, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_trans_epoch000.ckpt")
            break
