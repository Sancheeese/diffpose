...
class FrFTModule(nn.Module):
    def __init__(self, order=0.5, log_output=False, device_option='cuda', use_cache=True, learnable_order=False):
        ...
        """
        super(FrFTModule, self).__init__()
        ...
        self.calc_device = torch.device(device_option)
    ...
    def transform_vec(self, N, app_ord):
        ...
        with torch.no_grad():
            ...
            ec, vc = ec.real, vc.real
            es, vs = torch.linalg.eig(S2);
            es, vs = es.real, vs.real
    ...
    def FrFT2D(self, image):
        ...
        img = image.to(device).to(dtype=torch.complex64)
        img = torch.fft.fftshift(img, dim=(2, 3))

        out_h = torch.einsum('ij,bcjw->bciw', h_test, img)
        out_w = torch.einsum('ij,bchi->bchj', w_test, out_h)

        out = torch.fft.fftshift(out_w, dim=(2, 3))
