...
class Registration:
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            # else:
            #     loss = dice + self.criterion2(pred_img, self.img_ori)
            ...
            if self.i % 100 == 0:
                # a = self.criterion(pred_img, self.img_ori)
                # b = self.criterion(self.gt_img, self.img_ori)
                plt.figure()
                plt.imshow(pred_img_bone2.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                plt.show()
                # plt.figure()
            ...
            if self.i % 50 == 0:
                ...
                # true_fiducials, pred_fiducials = self.specimen.get_3d_tre(self.idx, pose)
    ...
    def run(self, idx):
        ...
        # plt.imshow(img_ori.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        # plt.show()
        initial_pose, _ = self.model(img, guide_mask=tube)
        ...
        overlay_grayscale_with_red_tensor(img_ori, edge, alpha=1)
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/zyl_800_norm_bone_mask3_pa_best_aug.ckpt", map_location="cuda:1")
    ...
    # specimen = IntubationDataset(root, x_root, y_offset=300, z_cut=650, factors=[1, 0.4, 0.4])
    ...
    # for idx in tqdm(range(86, len(specimen)), ncols=100):
    for idx in tqdm(range(2, len(specimen)), ncols=100):
        # if idx % 3 != 0:
        #     continue
        df = registration.run(idx)
        df.to_csv(
            f"runs/cma/cma_zyl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/cma/orinodicezyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
