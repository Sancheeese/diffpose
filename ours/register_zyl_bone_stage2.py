...
class Registration:
    def __init__(
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 150], [0.5, 0.5], device=self.device, step=[None, 50])
        # self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 32, 8], [0.1, 0.45, 0.45], device=self.device, step=[None, 32, 8])
        self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 32], [0.3, 0.7], device=self.device, step=[None, 8])
    ...
    def run(self, idx):
        ...
        for i in itr:
            ...
            mask = total_mask * (pred_img_bone.bool() | bone_mask_gt.bool()).float()
            # plt.figure()
            # plt.imshow(mask.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
            # plt.show()
            if i < self.n_iters / 2:
            # if i < 150:
                ...
                loss = self.criterion2(pred_img, img, mask)
