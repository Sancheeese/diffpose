...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            pred_pose = isocenter_pose.compose(back_pose).compose(offset).compose(center_pose)
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_800_norm_bone_mask3_center_best.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_800_norm_bone_mask3_center_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_800_norm_bone_mask3_center_best.ckpt")
            break
