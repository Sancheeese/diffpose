...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/wfl_800_norm_bone_mask3_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(0, 49), ncols=100):
        df = registration.run(idx)
        df.to_csv(
            f"runs/norm/wfl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/cma/nodicewfl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
