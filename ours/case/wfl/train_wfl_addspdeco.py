...
def load(height, device):
    ...
    specimen = IntubationDataset(root, x_root, x_offset=20, z_offset=50, z_cut=30, z_cut_end=250, factors=[0.7, 0.7, 1])
    ...
    for epoch in range(n_epochs + 1):
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/wfl_addspdeco_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/wfl_addspdeco_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/wfl_addspdeco_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/wfl_addspdeco_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/wfl_addspdeco_last.ckpt")
            break
