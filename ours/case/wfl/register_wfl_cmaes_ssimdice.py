...
class Registration:
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            sim = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * self.total_mask, toZeroOne(self.img_ori) * self.total_mask, window_size=11, reduction='mean')
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/wfl_coespdeco_best.ckpt", map_location="cuda:0")
    ...
    # for idx in tqdm(range(84, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/mask/ssimdicewfl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/cma/orinodicewfl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
