...
class Registration:
    ...
    def run(self, idx):
        ...
        for i in itr:
            ...
            ssim_ori = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * total_mask, toZeroOne(img_ori) * total_mask, window_size=11, reduction='mean')
            ssims.append(ssim.item())
            ssims_ori.append(ssim_ori.item())
            loss.backward()
            optimizer.step()
            scheduler.step()
        ...
        test = self.criterion(gt_img, img)
        ...
        ssims_ori.append((1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * total_mask, toZeroOne(img_ori) * total_mask, window_size=11, reduction='mean')).item())
        rot1.append(0)
        rot2.append(0)
        rot3.append(0)
        trans1.append(0)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/diff/img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        plt.axis('off')
        plt.savefig(f"./ret/diff/pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay = overlay_grayscale_with_red_tensor_save2(img_ori, edge, f"./ret/diff/overlay_{idx}.png", alpha=1)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/diff/wfl_diff_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        save_err_with_black(p, gt_img, f"./ret/diff/wfl_diff_black_{idx}.png", 119)
        save_err_half(pred_img, gt_img, img_ori, overlay, edge, "./ret/diff/wfl_diff", idx)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/wfl_800_norm_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(0, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/diff/wfl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
