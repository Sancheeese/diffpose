...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"wfl_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        save_err_with_black(p, gt_img, f"./ret/wfl_black_{idx}.png", 119)
        save_err_half(p, gt_img, img_ori, overlay, edge, "./ret/wfl", idx)
...
def main(id_number, parameterization):
    ...
    specimen = IntubationDataset(root, x_root, x_offset=20, z_offset=50, z_cut=30, z_cut_end=250, factors=[0.6, 0.6, 1])
