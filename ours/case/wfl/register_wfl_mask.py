...
from ours.utils.metrics_grad_ncc import MultiscaleGradientNCC2d
...
class Registration:
    def __init__(
        ...
        self.grad_ncc_metric = MultiscaleGradientNCC2d([21], [1], device=self.device, step=[1])
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([32, 13], [0.5, 0.5], device=self.device, step=[32, 1])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        plt.show()
        img_bone = self.drr_bone(None, None, None, pose=gt_pose)
        ...
        # registration = self.initialize_registration(gt_img, img_bone)
        # img = img_ori
        ...
        self.grad_ncc_metric.set_wei(bone)
        self.grad_ncc_metric.set_mask(total_mask)
        ...
        for i in itr:
            ...
            # dice = dice_coefficient_with_mask(img_bone, pred_img_bone, total_mask)
            grad_ncc = self.grad_ncc_metric(pred_img, img_ori)
            dice = dice_coefficient_with_mask(bone, pred_img_bone, total_mask)
            iou = compute_iou(bone, pred_img_bone, total_mask)
            # grad_ncc = gradient_ncc(pred_img, img_ori, total_mask)
            # mse = torch.mean((bone * total_mask - pred_img_bone * total_mask) ** 2, dim=[1, 2, 3])
            loss = self.criterion(pred_img, img_ori) + dice
            # loss = wei * self.criterion(toZeroOne(pred_img), toZeroOne(img_ori)) + (1 - wei) * dice
            # loss = wei * self.criterion(torch.log(toZeroOne(pred_img)) + 1, torch.log(toZeroOne(img_ori) + 1)) + (1 - wei) * dice
            # loss = wei * self.criterion(pred_img, img_ori) + (1 - wei) * grad_ncc
            # loss = wei * dice + (1 - wei) * grad_ncc
            ...
            mpd = torch.mean(mpd)
            # rot = gt_pose.get_rotation("euler_angles", "ZYX").detach().cpu().numpy()[0]
            # rot[0] += 0.1
            # xyz = gt_pose.get_translation().detach().cpu().numpy()[0]
            # rot = torch.tensor(rot, dtype=torch.float32, device=self.device, requires_grad=True)
            # xyz = torch.tensor(xyz, dtype=torch.float32, device=self.device, requires_grad=True)
            # new_pose = RigidTransform(rot, xyz, "euler_angles", "ZYX")
            # true_fiducials, pred_fiducials = self.specimen.get_3d_fiducials(idx, pred_pose)
            # mtre = torch.norm(true_fiducials - pred_fiducials, dim=-1)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/wfl_coespdeco_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/wfl_addspdeco_best.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(2, 30), ncols=100):
        ...
        df.to_csv(
            # f"runs/mask/wfl/localnccwfl_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/wfl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/wfl/wfl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/wfl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
