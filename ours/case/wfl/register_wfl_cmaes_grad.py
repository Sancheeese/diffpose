...
class Registration:
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            # loss = self.criterion_no_wei(pred_img, self.img_ori)
            loss = gradient_ncc(pred_img, self.img_ori, self.total_mask)
            # loss = dice + grad_ncc
...
def main(id_number, parameterization):
    ...
    # for idx in tqdm(range(84, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/mask/gradwfl_xray{idx:03d}_{parameterization}.csv",
