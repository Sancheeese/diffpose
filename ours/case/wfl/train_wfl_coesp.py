    ...
    for epoch in range(n_epochs + 1):
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/wfl_coesp_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/wfl_coesp_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/wfl_coesp_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/wfl_coesp_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            # main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/wfl_800_norm_bone_mask3_pa_best.ckpt")
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/wfl_coesp_last.ckpt")
            # main(batch_size=4, n_batches_per_epoch=200, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/wfl_trans_epoch000.ckpt")
            break
