...
class Registration:
    ...
    def run(self, idx):
        ...
        # img = img_ori
        optimizer1, scheduler1 = self.initialize_optimizer(registration, r_lr=7.5e-3, t_lr=5)
        optimizer2, scheduler2 = self.initialize_optimizer(registration, r_lr=7.5e-3, t_lr=5)
        ...
        for i in itr:
            ...
            # if i < 150:
                ...
                if i == self.n_iters / 2:
                    ...
                    print(f"配准一半需要{time.time() - st}")
                loss = self.criterion2(pred_img, img)
                # loss = (1 - weight) * gradient_ncc(pred_img, img, total_mask) + weight * self.criterion2(pred_img, img)
                # loss = self.criterion2(self.transforms(toZeroOne(pred_img)), self.transforms(toZeroOne(img)))
                # loss = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * total_mask, toZeroOne(img) * total_mask, window_size=11, reduction='mean')
                # loss = self.mIOU(pred_img_bone, bone_mask_gt)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/stage/cyx_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    for idx in tqdm(range(30, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/stage/cyx_stage_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
