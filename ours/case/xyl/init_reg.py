...
from ours.utils.model2 import PoseRegressor, PoseRegressorCat, PoseRegressorAttn, PoseRegressorAttnWei, \
    PoseRegressorCatCBAM, PoseRegressorAttnCBAM, PoseRegressorAttnNoWei, PoseRegressorCoe, PoseRegressorField, \
    PoseRegressorCoeSp, PoseRegressorSp, PoseRegressorMultiSp, PoseRegressorNoAttn, PoseRegressorSpCat
...
class Registration:
    ...
    def run(self, idx):
        ...
        # bone = read_seg("/home/zsr/project/diffpose/ours/bone_seg/xyl_4.nrrd").to(self.device)
        self.img_bone = bone
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/xyl_cat_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_aw_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_awc_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_noattn_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_noattn_epoch500.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_multi_epoch500.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_multi_best.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/xyl_coesp_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_spcat_best.ckpt", map_location="cuda:2")
    # ckpt = torch.load(f"checkpoints/xyl_sp_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_coe_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_field_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_nowei_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_cbam_best.ckpt", map_location="cuda:1")
    # model = PoseRegressorNoAttn(
    # model = PoseRegressorMultiSp(
    model = PoseRegressorCoeSp(
    # model = PoseRegressorSpCat(
    # model = PoseRegressorSp(
    # model = PoseRegressorCoe(
    # model = PoseRegressorField(
    ...
    all_results = []
    # for idx in tqdm(range(2, 88), ncols=100):
    for idx in tqdm(range(2, len(specimen)), ncols=100):
...
def calculate_and_save_statistics(all_results, parameterization):
    ...
    mpd_values = np.array([result['mpd'] for result in all_results])
    ...
    stats = {
        ...
        'mpd_mean': np.mean(mpd_values),
        'mpd_std': np.std(mpd_values),
    ...
    print(f"MPD: {stats['mpd_mean']:.2f} ± {stats['mpd_std']:.2f} mm")
