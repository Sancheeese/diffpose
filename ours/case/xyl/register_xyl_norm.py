...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/xyl_diff_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/xyl_diff_overlay_{idx}.png", alpha=1)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/xyl_800_norm_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_800_norm_bone_mask3_best_500.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/xyl_800_norm_bone_mask5_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(23, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/diff/xyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
