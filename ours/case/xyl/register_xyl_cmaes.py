...
class Registration:
    def __init__(
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 21], [0.5, 0.5], device=self.device, step=[None, 1])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([13], [1], device=self.device, step=[1])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            loss = 0.6 * self.criterion(pred_img, self.img_ori) + 0.4 * dice
    ...
    def run(self, idx):
        ...
        initial_pose, _ = self.model(img_ori, bone)
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/xyl_addspdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_addspdeco_epoch300.ckpt", map_location="cuda:0")
    ...
    # for idx in tqdm(range(84, len(specimen)), ncols=100):
        ...
        df.to_csv(
            ...
            # f"runs/cma/orinodicexyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
