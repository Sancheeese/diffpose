...
from ours.utils.model2 import PoseRegressorCat, PoseRegressorAttn, PoseRegressorAttnWei, \
...
class Registration:
    ...
    def run(self, idx):
        ...
        img_input = toZeroOne(img_ori) * bone
        img_input = self.transforms(img_input, reverse=False)
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/xyl_coedeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_catdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_fielddeco_best.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/xyl_bone_multi_best.ckpt", map_location=device_str)
    # ckpt = torch.load(f"checkpoints/xyl_coespdeco_epoch300.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_multideco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_multideco_epoch500.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_mscaledeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_mscaledeco_epoch300.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_multifrftdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_frftalldeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/xyl_frftmaxdeco_best.ckpt", map_location="cuda:0")
