...
from ours.utils.model2 import PoseRegressorCoe, PoseRegressorCoeSpDeco, PoseRegressorGSpDeco
...
class Registration:
    ...
    def run(self, idx):
        ...
        # video_writer = cv2.VideoWriter(f'video/xyl_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/xyl_img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        plt.axis('off')
        plt.savefig(f"./ret/xyl_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/xyl_overlay_{idx}.png", alpha=1)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/xyl_coespdeco_best.ckpt", map_location="cuda:0")
    ...
    nccs = []
    for idx in tqdm(range(23, len(specimen)), ncols=100):
    # for idx in tqdm(range(2, 88), ncols=100):
    # for idx in tqdm(range(2, 30), ncols=100):
        df, ncc = registration.run(idx)
        df.to_csv(
            # f"runs/mask/xyl/localnccxyl_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/xyl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/xyl/xyl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/xyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
