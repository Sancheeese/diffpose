...
class Registration:
    def __init__(
        ...
        self.criterion_no_wei = MultiscaleNormalizedCrossCorrelation2dNowei([None], [1], device=self.device, step=[1])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        for i in itr:
            ...
            loss = 0.5 * gradient_ncc(pred_img, img_ori, None) + 0.5 * dice
            # loss = self.criterion_no_wei(pred_img, img_ori)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/xyl_coespdeco_best.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(2, 30), ncols=100):
        ...
        df.to_csv(
            ...
            f"runs/mask/graddicexyl_xray{idx:03d}_{parameterization}.csv",
