...
from CT_dataset2 import Transforms
...
from CT_dataset2 import IntubationDataset, create_circle_mask, create_circle_mask_reverse, toZeroOne, \
    simple_projection
import kornia
import cv2
...
class Registration:
    def __init__(
        ...
        # self.style_change =  StyleChanger("/media/sda1/PersonalFiles/yx/project/diffpose/ours/cut/drr_style_solid_5_all/80_net_G.pth",
    ...
    def run(self, idx):
        ...
        # video_writer = cv2.VideoWriter(f'video/zt_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
        ...
        for i in itr:
            ...
            # loss = self.criterion2(pred_img, img_ori)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/zt_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/zt_800_norm_bone_mask3_best_500.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/zt_800_norm_bone_mask5_best.ckpt", map_location="cuda:1")
    ...
    specimen = IntubationDataset(root, x_root, y_offset=50, z_offset=50, z_cut=550, factors=[0.5, 4, 0.5])
    ...
    for idx in tqdm(range(20, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/zt_stage_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
