...
from ours.utils.model2 import PoseRegressorCoe, PoseRegressorCoeSpDeco, PoseRegressorGSpDeco, PoseRegressorAddSpDeco
...
from CT_dataset_iso import IntubationDataset
import kornia
import cv2
...
class Registration:
    def __init__(
        ...
        self.grad_ncc_metric = MultiscaleGradientNCC2d([64], [1], device=self.device, step=[64])
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([32, 13], [0.5, 0.5], device=self.device, step=[8, 1])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        # bone = read_seg("/home/zsr/project/diffpose/ours/bone_seg/sxh_4.nrrd").to(self.device)
        self.img_bone = bone
        ...
        img_input = high_pass_gaussian(img_ori, 0.1)
        ...
        # video_writer = cv2.VideoWriter(f'video/sxh_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/sxh_addspdeco_best.ckpt", map_location="cuda:1")
    ckpt = torch.load(f"checkpoints/sxh_addspdeco_fft_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/sxh_coespdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/sxh_coespdeco_en_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/sxh_coespdeco_epoch300.ckpt", map_location="cuda:0")
    ...
    specimen = IntubationDataset(root, x_root, x_offset=20, y_offset=200, z_offset=100, z_cut=250,
                                 factors=[0.6, 0.6, 1.5])
    height = ckpt["height"]
    ...
    nccs = []
    for idx in tqdm(range(17, len(specimen)), ncols=100):
    # for idx in tqdm(range(3, len(specimen)), ncols=100):
    # for idx in tqdm(range(0, 50), ncols=100):
    # for idx in tqdm(range(2, 30), ncols=100):
        df, ncc = registration.run(idx)
        df.to_csv(
            # f"runs/mask/sxh/localnccsxh_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/sxh_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/sxh/sxh_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/sxh_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
