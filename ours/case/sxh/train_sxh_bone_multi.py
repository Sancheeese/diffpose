    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            if loss.isnan().any():
                ...
                #         **model_params,
                #     },
                #     f"checkpoints/sxh_crashed.ckpt",
                # )
                print("Nan loss")
                continue
                # raise Exception("sad...")
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/sxh_bone_multi_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/sxh_bone_multi_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/sxh_bone_multi_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/sxh_bone_multi_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            # main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/sxh_800_norm_bone_mask3_pa_best.ckpt")
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/sxh_bone_multi_last.ckpt")
            break
