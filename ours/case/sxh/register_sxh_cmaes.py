...
class Registration:
    def __init__(
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([32], [1], device=self.device, step=[4])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
        self.criterion_singlewei = MultiscaleNormalizedCrossCorrelation2dSingleWei([32], [1], device=self.device, step=[4])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
        ...
        self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_nec5/70_net_G.pth",
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/sxh_addspdeco_best_bak.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/sxh_addspdeco_best.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/sxh_addspdeco_fft_best.ckpt", map_location="cuda:0")
    ...
    # for idx in tqdm(range(84, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/mask/cma/sxh_xray{idx:03d}_{parameterization}.csv",
            # f"runs/cma/orinodicesxh_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
