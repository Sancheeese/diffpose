...
class Registration:
    ...
    def run(self, idx):
        ...
        # bone = read_seg("/home/zsr/project/diffpose/ours/bone_seg/cyx_4.nrrd").to(self.device)
        self.img_bone = bone
        ...
        for i in itr:
            ...
            loss = 0.8 * self.criterion_no_wei(pred_img, img_ori) + 0.2 * dice
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/cyx_coespdeco_best.ckpt", map_location="cuda:0")
    ...
    # for idx in tqdm(range(2, 30), ncols=100):
        ...
        df.to_csv(
            # f"runs/mask/cyx/localncccyx_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/cyx/cyx_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/cyx/cyx_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/cyx_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
