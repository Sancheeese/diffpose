...
class Registration:
    ...
    def run(self, idx):
        ...
        # img = img_ori
        optimizer, scheduler = self.initialize_optimizer(registration, r_lr=20e-3, t_lr=10)
        ...
        # video_writer = cv2.VideoWriter(f'video/cyx_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
        ...
        for i in itr:
            ...
            # loss = self.criterion2(pred_img, img, None, wei_img)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/cyx_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/cyx_800_norm_bone_mask3_best_500.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_800_norm_bone_mask5_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(30, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/cyx_stage_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
