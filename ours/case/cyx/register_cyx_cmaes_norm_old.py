...
class Registration:
    ...
    def run(self, idx):
        ...
        white_mask = torch.ones_like(img)
        white_mask[toZeroOne(img) > 0.8] = 1
        plt.figure()
        plt.imshow(circle_mask.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        plt.show()
        white_mask = white_mask * circle_mask
        ...
        initial_pose, _ = self.model(flip_img_w(tmp))
        ...
        # r_l2 = 5e-1
        # t_l2 = 20
        # s2 = [r_l2, r_l2, r_l2, t_l2, t_l2, t_l2]
        # option2 = {
        #     # "seed": 123,
        #     # "CMA_rankmu": 0.2,
        #     "popsize": 15,
        ...
        # }
        #
        # optimizer2 = cma.CMAEvolutionStrategy(x0=x, sigma0=1, options=option2)
        # optimizer2.optimize(self.nlopt_objective)
        #
        # # 5. 输出结果
        # print("二阶段最优解:", optimizer2.result.xbest)
        # print("二阶段目标值:", optimizer2.result.fbest)
