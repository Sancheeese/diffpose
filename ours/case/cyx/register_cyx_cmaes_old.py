...
from ours.utils.img_utils import flip_img_w
...
from CT_dataset_old import IntubationDataset, create_circle_mask
...
class Registration:
    def __init__(
        ...
        # self.style_change = StyleChanger("/media/sda1/PersonalFiles/yx/project/diffpose/ours/cut/ckpt/70_net_G.pth",
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            loss = weight * dice + (1 - weight) * gradient_ncc(pred_img, self.img, self.total_mask) + weight * self.criterion(pred_img, self.img)
            ...
            # loss = self.criterion(pred_img, self.img, None, self.wei_img)
    ...
    def run(self, idx):
        ...
        white_mask = torch.ones_like(img)
        white_mask[toZeroOne(img) > 0.8] = 0
        plt.figure()
        plt.imshow(circle_mask.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        plt.show()
        white_mask = white_mask * circle_mask
        ...
        initial_pose, _ = self.model(flip_img_w(img))
        ...
        mask_bone = flip_img_w(mask_bone)
        plt.figure()
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/cma/cyx_stage_xray{idx:03d}.jpg")
        plt.show()
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/cyx_800_norm_best_afe.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_epoch050.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_unknow.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_tube_no_change_best2.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_tube_no_change_best_shallow.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_800_norm_best.ckpt", map_location="cuda:1")
    ckpt = torch.load(f"checkpoints/cyx_800_norm_bone_mask3_best2.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/cyx_mncc_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(28, len(specimen)), ncols=100):
        df = registration.run(idx)
        df.to_csv(
            f"runs/cma/cyx_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
