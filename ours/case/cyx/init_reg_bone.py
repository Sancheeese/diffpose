...
class Registration:
    def __init__(
        ...
        # self.style_change =  StyleChanger("/media/sda1/PersonalFiles/yx/project/diffpose/ours/cut/drr_style_solid_5_all/150_net_G.pth",
        # self.style_change =  StyleChanger("/media/sda1/PersonalFiles/yx/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_5_iso/90_net_G.pth",
        # self.style_change =  StyleChanger("/media/sda1/PersonalFiles/yx/project/diffpose/ours/cut/drr_style_solid_5_new4/150_net_G.pth",
    ...
    def evaluate(self, registration):
        ...
        a = self.doublegeo(est_pose, self.pose)
        b = self.geodesics(est_pose, self.pose)
        geo = (
            torch.concat(
                [
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/cyx_800_norm_bone_mask3_best.ckpt", map_location="cuda:1")
    ...
    specimen = IntubationDataset(root, x_root, z_offset=70, factors=[0.5, 0.5, 1])
    ...
    all_results = []
    # for idx in tqdm(range(2, 6), ncols=100):
...
def calculate_and_save_statistics(all_results, parameterization):
    ...
    detailed_df.to_csv(f"initial_pose_detailed_results_bone.csv", index=False)

    # 保存统计结果
    stats_df = pd.DataFrame([stats])
    stats_df.to_csv(f"initial_pose_statistics_bone.csv", index=False)
    ...
    print(f"\n--- 几何误差统计 ---")
    print(f"旋转误差 (geo_r): {stats['geo_r_mean']:.4f} ± {stats['geo_r_std']:.4f} mm")
    print(f"平移误差 (geo_t): {stats['geo_t_mean']:.4f} ± {stats['geo_t_std']:.4f} mm")
    print(f"双几何误差 (geo_d): {stats['geo_d_mean']:.4f} ± {stats['geo_d_std']:.4f}")
    print(f"SE3几何误差: {stats['geo_se3_mean']:.4f} ± {stats['geo_se3_std']:.4f}")

    print(f"\n--- 图像质量指标 ---")
    print(f"SSIM (生成图像): {stats['ssim_mean']:.4f} ± {stats['ssim_std']:.4f}")
    print(f"SSIM (原始图像): {stats['ssim_ori_mean']:.4f} ± {stats['ssim_ori_std']:.4f}")

    print(f"\n--- 目标配准误差 ---")
    print(f"TRE: {stats['tre_mean']:.4f} ± {stats['tre_std']:.4f} mm")

    print(f"\n--- 初始参数统计 ---")
    print(f"Alpha: {stats['alpha_mean']:.4f} ± {stats['alpha_std']:.4f}")
    print(f"Beta: {stats['beta_mean']:.4f} ± {stats['beta_std']:.4f}")
    print(f"Gamma: {stats['gamma_mean']:.4f} ± {stats['gamma_std']:.4f}")
    print(f"Bx: {stats['bx_mean']:.4f} ± {stats['bx_std']:.4f}")
    print(f"By: {stats['by_mean']:.4f} ± {stats['by_std']:.4f}")
    print(f"Bz: {stats['bz_mean']:.4f} ± {stats['bz_std']:.4f}")

    print(f"\n--- 预测参数统计 ---")
    print(f"Alpha2: {stats['alpha2_mean']:.4f} ± {stats['alpha2_std']:.4f}")
    print(f"Beta2: {stats['beta2_mean']:.4f} ± {stats['beta2_std']:.4f}")
    print(f"Gamma2: {stats['gamma2_mean']:.4f} ± {stats['gamma2_std']:.4f}")
    print(f"Bx2: {stats['bx2_mean']:.4f} ± {stats['bx2_std']:.4f}")
    print(f"By2: {stats['by2_mean']:.4f} ± {stats['by2_std']:.4f}")
    print(f"Bz2: {stats['bz2_mean']:.4f} ± {stats['bz2_std']:.4f}")
    print("=" * 80)
