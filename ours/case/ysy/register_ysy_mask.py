...
from ours.fft import high_pass_gaussian
...
from ours.utils.img_utils import flip_img_w, overlay_grayscale_with_red_tensor, overlay_grayscale_with_red_tensor_save
...
from ours.utils.model2 import PoseRegressorCoe, PoseRegressorCoeSpDeco, PoseRegressorAddSpDeco
...
class Registration:
    def __init__(
        ...
        self.back_pose = specimen.back_pose.to(self.device)
        self.center_pose = specimen.center_pose.to(self.device)
        ...
        # self.criterion_no_wei = MultiscaleNormalizedCrossCorrelation2dNowei([None, 21], [0.5, 0.5], device=self.device, step=[None, 1])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        # bone = read_seg("/home/zsr/project/diffpose/ours/bone_seg/ysy_4.nrrd").to(self.device)
        self.img_bone = bone
        ...
        img_input = high_pass_gaussian(img_ori, 0.2)
        # img_input = toZeroOne(img_input) * tube
        ...
        # img = flip_img_w(img)
        registration = self.initialize_registration(img_input, bone)
        # img = img_ori
        ...
        # video_writer = cv2.VideoWriter(f'video/ysy_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
        ...
        for i in itr:
            ...
            # mse = torch.mean((bone * total_mask - pred_img_bone * total_mask) ** 2, dim=[1, 2, 3])
            loss = wei * self.criterion(pred_img, img_ori) + (1 - wei) * dice
            # loss = wei * self.criterion(pred_img, img_ori) + (1 - wei) * (1 - mse)
            # loss = self.criterion(pred_img, img_ori) - 10 * mse
            # loss = gradient_ncc(pred_img, img_ori, total_mask) + self.criterion(pred_img, img_ori)
            ...
            mpd = torch.mean(mpd)
            mtre = self.specimen.calc_tre(idx, pred_pose)
        ...
        p = self.drr(None, None, None, pose=pred_pose, bone_attenuation_multiplier=3)
        p = self.transforms(p).to(self.device)
        plt.figure(dpi=300)
        plt.imshow(img_ori.cpu().squeeze(), cmap="gray")
        plt.axis('off')
        plt.savefig(f"./ret/ysy_img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        plt.figure(dpi=300)
        plt.imshow(p.detach().cpu().squeeze(), cmap="gray")
        plt.axis('off')
        plt.savefig(f"./ret/ysy_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/ysy_overlay_{idx}.png", alpha=1)
        ...
        dice = dice_coefficient_with_mask(bone, img_bone, total_mask)
        loss = wei * self.criterion(gt_img, img_ori) + (1 - wei) * dice
        losses.append(loss.item())
        times.append(0)
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/ysy_coespdeco_en_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/ysy_addspdeco_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/ysy_coespdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/ysy_coespdeco_epoch300.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/ysy_addspdeco_fft_best.ckpt", map_location="cuda:0")
    ...
    specimen = IntubationDataset(root, x_root, y_offset=50, z_offset=-50, z_cut=400, factors=[0.5, 0.5, 0.5])
    height = ckpt["height"]
    ...
    # for idx in tqdm(range(2, 30), ncols=100):
        ...
        df.to_csv(
            # f"runs/mask/ysy/localnccysy_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/ysy_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/ysy/ysy_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/ysy_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
