...
from ours.utils.registration_bone_offset import SparseRegistration
...
class Registration:
    ...
    def initialize_registration(self, img, mask):
        ...
        pred_pose = self.back_pose.inverse().compose(self.isocenter_pose.inverse()).compose(pred_pose).compose(self.center_pose.inverse())
    ...
    def evaluate(self, registration):
        ...
        rot = torch.tensor([param[:3]], dtype=torch.float32, device=self.device)
        xyz = torch.tensor([param[3:]], dtype=torch.float32, device=self.device)
    ...
    def run(self, idx):
        ...
        plt.title(f"NO.{idx}")
        plt.show()
        pose_params = [0.3314078152179718, -0.20976108312606812, -0.0659443810582161, -2.4527926445007324,
                       3.0570242404937744, 17.762699127197266]
        ...
        # self.pose = RigidTransform(rot, xyz, parameterization="so3_log_map", device=self.device)
        pose = RigidTransform(rot, xyz, "euler_angles", "ZYX", device=self.device)
        ...
        for i in itr:
            ...
            optimizer.zero_grad()
            pred_img, pred_img_bone, mask, pred_pose = registration(self.isocenter_pose, self.back_pose, self.center_pose)
