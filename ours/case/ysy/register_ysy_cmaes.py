...
from ours.utils.metrics_mask_tube2_weiweiwei import MultiscaleNormalizedCrossCorrelation2d as MultiscaleNormalizedCrossCorrelation2dSingleWei
from ours.utils.metrics_fft_ncc import MultiscaleFFTNCC2d
...
class Registration:
    def __init__(
        ...
        self.criterion_fft = MultiscaleFFTNCC2d([None], [1], device=self.device, step=[None])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
        ...
        self.criterion_singlewei = MultiscaleNormalizedCrossCorrelation2dSingleWei([32], [1], device=self.device, step=[32])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
        ...
        self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_nec5/50_net_G.pth",
    ...
    def nlopt_objective(self, x):
        with torch.no_grad():
            ...
            pose = self.pred_pose.compose(pose)
            ...
            bone1 = torch.tensor(pred_img_bone1)
            bone1[bone1 < 0.01] = 0
            bone1[bone1 >= 0.01] = 1
            bone = (bone1.bool() | self.img_bone.bool()).float()
            ...
            # loss = self.criterion_singlewei(pred_img, self.img_ori, bone) + dice
            # loss = self.criterion_fft(pred_img, self.img_ori) + dice
            # loss = self.criterion_no_wei(pred_img, self.img_ori) + dice
            # grad_ncc = gradient_ncc(pred_img, self.img_ori, self.total_mask)
            # loss = self.criterion(pred_img, self.img_ori) + grad_ncc
    ...
    def run(self, idx):
        ...
        plt.show()
        plt.figure(dpi=300)
        plt.imshow(img_input.cpu().squeeze(), cmap="gray")
        plt.axis('off')
        plt.show()
        ...
        self.criterion_singlewei.set_mask(total_mask)
        self.criterion_fft.set_wei(bone)
        self.criterion_fft.set_mask(total_mask)
        ...
        loss = self.criterion_singlewei(gt_img, self.img_ori, bone)
        # a = torch.zeros_like(img).to(self.device).to(torch.float32)
        initial_pose, _ = self.model(img_input, bone)
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/ysy_addspdeco_best.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/ysy_addspdeco_fft_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/ysy_addspdeco_epoch300.ckpt", map_location="cuda:0")
    ...
    specimen = IntubationDataset(root, x_root, y_offset=50, z_offset=-50, z_cut=400, factors=[0.8, 1.5, 0.8])
    ...
    # for idx in tqdm(range(84, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/mask/cma/ysy_xray{idx:03d}_{parameterization}.csv",
            # f"runs/cma/orinodiceysy_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
