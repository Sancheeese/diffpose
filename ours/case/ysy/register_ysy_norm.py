...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 32], [0.5, 0.5], device=self.device, step=[None, 1])
        # self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 8], [0.5, 0.5], device=self.device, step=[None, 4])
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/ysy_diff_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/ysy_diff_overlay_{idx}.png", alpha=1)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/ysy_norm_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(9, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/diff/ysy_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
