...
class Registration:
    ...
    def run(self, idx):
        ...
        # gt_img = self.style_change(gt_img)
        # gt_img = self.transforms(gt_img, reverse=False).to(self.device).to(torch.float32)
        self.gt_img = gt_img
        plt.figure()
        ...
        pose_params = [-0.25, 0.15, 0.1, -40, 180, 20]
        ...
        pose = self.isocenter_pose.compose(center_pose).compose(pose).compose(back_pose)
        test_img = self.drr(None, None, None, pose=pose)
        test_img = self.transforms(test_img).to(self.device).to(torch.float32)
        plt.figure()
        plt.imshow(test_img.cpu().squeeze(), cmap="gray")
        plt.show()

        initial_pose, _ = self.model(test_img)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/cma/yjj_stage_xray{idx:03d}.jpg")
        plt.show()
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/yjj_800_norm_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(10, len(specimen)), ncols=100):
        df = registration.run(idx)
        df.to_csv(
            f"runs/cma/yjj_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
