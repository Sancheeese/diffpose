...
from ours.utils.img_utils import inpaint_with_opencv, overlay_grayscale_with_red_tensor
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/jjl_800_norm_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(30, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/cma/jjl_xray{idx:03d}_{parameterization}.csv",
            # f"runs/cma/nodicejjl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
