...
class Registration:
    ...
    def run(self, idx):
        ...
        # video_writer = cv2.VideoWriter(f'video/jjl_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/stage/jjl_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/jjl_800_norm_bone_mask3_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/jjl_800_norm_bone_mask3_best_500.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/jjl_800_norm_bone_mask5_best.ckpt", map_location="cuda:1")
    ...
    root = "/home/zsr/project/diffpose/ours/data/liwei/江莲娇/CT/JiangLianJiao/20240716162518.679/602"
    # root = "/media/sda1/Data/ERCP/CT+X+MRCP/liwei/江莲娇/CT/JiangLianJiao/20240716162518.679/602"
    x_root = "/home/zsr/project/diffpose/ours/data/liwei/江莲娇/ERCP/JIANGLIANJIAO^^/20240724145013/1"
    # x_root = "/media/sda1/Data/ERCP/CT+X+MRCP/liwei/江莲娇/ERCP/JIANGLIANJIAO^^/20240724145013/1"
    specimen = IntubationDataset(root, x_root, x_offset=-60, z_offset=150, factors=[0.5, 4, 0.5])
    ...
    for idx in tqdm(range(30, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/stage/jjl_stage_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
