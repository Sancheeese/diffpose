...
class Registration:
    ...
    def run(self, idx):
        ...
        # initial_pose, _ = self.model(img, guide_mask=total_mask)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/cma/xyl_stage_xray{idx:03d}.jpg")
        plt.show()
...
def main(id_number, parameterization):
    ...
    ckpt = torch.load(f"checkpoints/qbt_800_norm_bone_mask3_best.ckpt", map_location="cuda:1")
