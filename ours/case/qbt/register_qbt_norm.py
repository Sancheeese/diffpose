...
class Registration:
    ...
    def run(self, idx):
        ...
        # # file_path = f"/media/sda1/PersonalFiles/yx/dataset/xyl_result/{filename}.nii.gz"
        # file_path = f"nnuet/xyl_result/{filename}.nii.gz"
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/xyl_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    for idx in tqdm(range(30, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/xyl_stage_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
