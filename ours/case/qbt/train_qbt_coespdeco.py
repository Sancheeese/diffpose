...
from ours.utils.model2 import PoseRegressor, PoseRegressorCat, PoseRegressorAttnWei, PoseRegressorAttnNoWei, \
    PoseRegressorCoe, PoseRegressorCoeDeco, PoseRegressorCoeSpDeco, PoseRegressorGSpDeco
...
def load(height, device):
    root = "/home/zsr/project/diffpose/ours/data/liwei/邱碧潭/CT/QiuBiTan/20240425223050.648000/3"
    x_root = "/home/zsr/project/diffpose/ours/data/liwei/邱碧潭/ERCP/BITANG^QIU^/20240426153102/1"
    # root = "/media/sda1/Data/ERCP/CT+X+MRCP/liwei/邱碧潭/CT/QiuBiTan/20240425223050.648000/3"
    # x_root = "/media/sda1/Data/ERCP/CT+X+MRCP/liwei/邱碧潭/ERCP/BITANG^QIU^/20240426153102/1"

    # specimen = IntubationDataset(root, x_root, y_offset=125, z_offset=125, factors=[0.5, 3, 0.5])
    specimen = IntubationDataset(root, x_root, y_offset=125, z_offset=125, factors=[0.8, 1.5, 0.8])
...
def train(
    ...
    # metric = MultiscaleNormalizedCrossCorrelation2d(eps=1e-4, patch_sizes=[None, 13], patch_weights=[0.5, 0.5], device=device)
    ...
    # augmenter = MaskDataAugmentation("/media/sda1/PersonalFiles/yx/project/diffpose/ours/tube")
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            img_bone = transforms(img_bone, reverse=False)
            ...
            img_bone_tube = torch.tensor(img_bone).to(device)
            is_tube = False
            if random.random() < 0.7:
                img, tube = augmenter.augment(img)
                img_bone_tube = img_bone_tube * tube
                is_tube = True
            ...
            pred_offset, pred_mask = model(img, img_bone_tube)
            # pred_offset, pred_mask = model(img, img_bone)
            # a = pred_offset.get_rotation()
            # b = pred_offset.get_translation()
            # print(a)
            # print(b)
            ...
            if is_tube:
                pred_img = toZeroOne(pred_img) * tube
                pred_img = transforms(pred_img, reverse=False)
            # for im in pred_img:
            #     plt.figure()
            ...
            mse = torch.mean((pred_mask - img_bone) ** 2, dim=[1,2,3])

            loss = 1 - ncc + 1e-2 * (double_geodesic + log_geodesic) + 10 * mse
            # loss = 1 - ncc + 1e-2 * (double_geodesic + log_geodesic)
            if loss.isnan().any():
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/qbt_coespdeco_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/qbt_coespdeco_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/qbt_coespdeco_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    model = PoseRegressorGSpDeco(**model_params)
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/qbt_coespdeco_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/qbt_coespdeco_last.ckpt")
            break
