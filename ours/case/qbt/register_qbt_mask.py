...
class Registration:
    ...
    def run(self, idx):
        ...
        # bone = read_seg("/home/zsr/project/diffpose/ours/bone_seg/qbt_4.nrrd").to(self.device)
        self.img_bone = bone
        ...
        # video_writer = cv2.VideoWriter(f'video/qbt_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
...
def main(id_number, parameterization):
    # ckpt = torch.load(f"checkpoints/qbt_coespdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/qbt_coespdeco_en_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/qbt_coespdeco_epoch500.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/qbt_addspdeco_best.ckpt", map_location="cuda:0")
    ...
    # for idx in tqdm(range(2, 30), ncols=100):
        ...
        df.to_csv(
            # f"runs/mask/qbt/localnccqbt_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/qbt_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/qbt/qbt_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/qbt_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
