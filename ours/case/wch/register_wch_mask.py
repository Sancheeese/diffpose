...
class Registration:
    def __init__(
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([64, 16], [0.5, 0.5], device=self.device, step=[32, 8])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        for i in itr:
            ...
            grad_ncc = gradient_ncc(img_ori, pred_img, total_mask)
            ...
            # loss = w * self.criterion(pred_img, img_ori) + w * grad_ncc + w * dice
