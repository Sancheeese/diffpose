...
from ours.utils.img_utils import get_streak_artifacts
...
from ours.utils.CT_dataset_augment2 import Transforms as TransForms_augment
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            if random.random() < 0.9:
                tube = 1 - get_streak_artifacts(
                    height=256,
                    width=256,
                    num_streaks_range=(2, 4),
                    width_range=(0.05, 0.13),
                    length_range=(0.6, 1.0),
                    shape_types=["straight", "curved", "curved2"],
                )
                tube = torch.tensor(tube).to(device)
                img_bone_tube *= tube
                # is_tube = True
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/wch_coespdeco_en_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/wch_coespdeco_en_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/wch_coespdeco_en_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/wch_coespdeco_en_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/wch_coespdeco_en_last.ckpt")
            break
