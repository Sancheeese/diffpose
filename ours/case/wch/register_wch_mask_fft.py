...
class Registration:
    ...
    def run(self, idx):
        ...
        # video_writer = cv2.VideoWriter(f'video/wch_{idx}.mp4', fourcc, 30, (256, 256), isColor=False)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/wch_img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        plt.figure(dpi=300)
        plt.imshow(p.detach().cpu().squeeze(), cmap="gray")
        plt.axis('off')
        plt.savefig(f"./ret/wch_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/wch_overlay_{idx}.png", alpha=1)
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/wch_addspdeco_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/wch_coespdeco_best.ckpt", map_location="cuda:0")
    # ckpt = torch.load(f"checkpoints/wch_coespdeco_epoch300.ckpt", map_location="cuda:0")
    ckpt = torch.load(f"checkpoints/wch_addspdeco_fft_best.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(2, 30), ncols=100):
        ...
        df.to_csv(
            # f"runs/mask/wch/localnccwch_xray{idx:03d}_{parameterization}.csv",
            f"runs/mask/wch_xray{idx:03d}_{parameterization}.csv",
            # f"runs/mask/wch/wch_xray{idx:03d}_{parameterization}.csv",
            # f"runs/grad/wch_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
