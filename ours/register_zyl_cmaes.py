...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 128], [0.5, 0.5], device=self.device, step=[None, 128 // 2])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        tube = torch.ones_like(img_ori).to(self.device)
        tube[img_ori < 0.25] = 0
        plt.figure()
        ...
        img_ori = inpaint_with_opencv(img_ori, black)
        img_ori = self.transforms(img_ori, reverse=False).to(self.device).to(torch.float32)
...
def main(id_number, parameterization):
    ...
    for idx in tqdm(range(22, len(specimen)), ncols=100):
