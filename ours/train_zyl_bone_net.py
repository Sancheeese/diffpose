...
from utils.registration_feat import PoseRegressor
...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            img_bone = drr_bone(None, None, None, pose=pose, bone_attenuation_multiplier=contrast)
            ...
            img_bone_mask = torch.tanh(30 * img_bone)
            img_bone = torch.tanh(5 * img_bone)
            # for im in img_bone:
            #     plt.figure()
            ...
            mask1 = F.interpolate(img_bone_mask, layer1.shape[-2:], mode='bilinear')
            mask2 = F.interpolate(img_bone_mask, layer2.shape[-2:], mode='bilinear')
            layer1 = (layer1 * mask1).mean(dim=1, keepdim=True)
            layer1 = (layer1 - layer1.min()) / (layer1.max() - layer1.min() + 1e-8)
            layer2 = (layer2 * mask2).mean(dim=1, keepdim=True)
            layer2 = (layer2 - layer2.min()) / (layer2.max() - layer2.min() + 1e-8)
            bone1 = F.interpolate(img_bone, layer1.shape[-2:], mode='bilinear')
            bone2 = F.interpolate(img_bone, layer2.shape[-2:], mode='bilinear')
            # for im in bone1:
            #     plt.figure()
            #     plt.imshow(im.cpu().permute(1, 2, 0), cmap='gray')
            #     plt.show()
            ...
            #     plt.imshow(im.cpu().permute(1, 2, 0), cmap='gray')
            #     plt.show()
            fl1 = torch.mean((layer1 - bone1) ** 2)
            fl2 = torch.mean((layer2 - bone2) ** 2)
            feat_loss = fl1 + fl2

            loss = 1 - ncc + 1e-2 * (log_geodesic + double_geodesic) - 10 * feat_loss
            if loss.isnan().any():
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_800_norm_bone_net3_best.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_800_norm_bone_net3_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_800_norm_bone_net3_best.ckpt")
            break
