...
def process_csv_file(csv_file, specimen, style_change, drr, drr_bone, height, device="cuda:0"):
    ...
    plt.figure()
    plt.imshow(p.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
    plt.title(f"NO.{id_number}---tre={tre.item()}")
    plt.show()
...
if __name__ == "__main__":
    ...
    # directory_path = f"/home/zsr/project/diffpose/ours/case/xyl/runs/mask/xyl" # 替换为你的CSV文件目录
    ...
    directory_path = "/home/zsr/project/diffpose/ours/case/ysy/runs/mask/cma"  # 替换为你的目录路径
