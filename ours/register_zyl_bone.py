...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.3, 0.4, 0.3], device=self.device, step=[None, 1, 1])
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.1, 0.45, 0.45], device=self.device)
    ...
    def run(self, idx):
        ...
        for i in itr:
            ...
            # fiducial.append(tre)
            times.append(t1 - t0)
            ssim = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * total_mask, toZeroOne(img) * total_mask,
        ...
        plt.title(f"no.{idx}")
        plt.savefig(f"reg_result/no_stage/zyl_no_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
