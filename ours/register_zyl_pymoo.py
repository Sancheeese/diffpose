...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 30, 11], [0.3, 0.4, 0.3], device=self.device)
    ...
    def nlopt_objective(self, x):
        ...
        # plt.figure()
        # plt.imshow(self.bone_mask_gt.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        # plt.show()
        ...
        #
        #
        # intersection = (pred_img_bone * self.bone_mask_gt * self.total_mask).sum()
        # union = (pred_img_bone * self.total_mask).sum() + (self.bone_mask_gt * self.total_mask).sum() - intersection
    ...
    def run(self, idx):
        ...
        self.bone_mask_gt = bone_mask_gt
        plt.figure()
        ...
        plt.show()
        self.drr_bone(None, None, None, pose=initial_pose, bone_attenuation_multiplier=3)
        ...
        lb = np.array([-np.pi, -np.pi, -np.pi, -300.0, -300.0, -300.0])  # SE(3)下界
        ub = np.array([np.pi, np.pi, np.pi, 300.0, 300.0, 300.0])
        ...
        # result = optimize.minimize(self.nlopt_objective, initial_params, method="L-BFGS-B", options=lbfgsb_options, bounds=bounds, jac=True)
        result = basinhopping(
            self.nlopt_objective,
            initial_params,
            niter=100,
            T=1.0,
            stepsize=0.1,
        ...
        img = self.drr(None, None, None, pose=final_pose, bone_attenuation_multiplier=3)
        img = self.transforms(img).to(self.device)
        plt.figure()
        plt.imshow(img.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        plt.show()
