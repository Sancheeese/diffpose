...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            loss = 1 - ncc + 1e-2 * (log_geodesic + double_geodesic) + 10 * mse
            if loss.isnan().any():
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/specimen_03_coespdeco_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/specimen_03_coespdeco_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/specimen_03_coespdeco_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    specimen, isocenter_pose, transforms, drr, drr_bone = load(3, height, device)
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/specimen_03_coespdeco_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            # main(batch_size=4, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/specimen_03_coespdeco_last.ckpt")
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/specimen_03_coespdeco_best.ckpt")
            break
