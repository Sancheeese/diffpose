...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/specimen_{id_number:02d}_best.ckpt", map_location="cuda:1")
    model = PoseRegressor(
        ckpt["model_name"],
    ...
    for idx in tqdm(range(0, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/1/norm/specimen{id_number:02d}_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
