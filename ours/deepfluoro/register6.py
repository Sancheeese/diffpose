...
from ours.utils.grad_similar import get_edge
...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig("./ret/6_diff_pred_img.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img, edge, "./ret/6_diff_overlay.png", alpha=1)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/6_diff_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        plt.figure(dpi=300)
        plt.imshow(img.detach().cpu().squeeze(), cmap="gray")
        plt.axis('off')
        plt.savefig(f"./ret/6_diff_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/specimen_{id_number:02d}_best.ckpt", map_location="cuda:0")
    model = PoseRegressor(
        ckpt["model_name"],
    ...
    for idx in tqdm(range(0, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/{id_number}/norm/specimen{id_number:02d}_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
