...
class Registration:
    ...
    def initialize_optimizer(self, registration):
        optimizer = torch.optim.Adam(
            [
                {"params": [registration.rotation], "lr": 5e-3},
    ...
    def run(self, idx):
        ...
        plt.figure()
        plt.imshow(gt_img.cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        plt.title(f"NO.{idx}")
        plt.show()
        ...
        plt.axis('off')
        plt.savefig("./ret/6_pred_img.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img, edge, "./ret/6_overlay.png", alpha=1)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/6_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        plt.figure(dpi=300)
        plt.imshow(img.detach().cpu().squeeze(), cmap="gray")
        plt.axis('off')
        plt.savefig(f"./ret/6_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/specimen_06_coespdeco_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(0, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/mask/{id_number}/specimen{id_number:02d}_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
