...
class Registration:
    ...
    def run(self, idx):
        ...
        bone = get_bone(2, idx).to(self.device).to(torch.float32)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/specimen_02_coespdeco_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(0, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/mask/2/specimen{id_number:02d}_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
