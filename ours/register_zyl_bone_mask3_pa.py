...
class Registration:
    ...
    def run(self, idx):
        ...
        for i in itr:
            ...
            dice = dice_coefficient_with_mask(pred_img_bone, mask_bone, total_mask)
            # loss = weight * dice + weight * self.criterion2(pred_img, img)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/no_stage/zyl_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/zyl_epoch800.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(62, 66), ncols=100):
        ...
        df.to_csv(
            f"runs/grad/orinodicezyl_xray{idx:03d}_{parameterization}.csv",
