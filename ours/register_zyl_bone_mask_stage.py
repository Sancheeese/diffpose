...
from utils.metrics_mask_tube2_wei import MultiscaleNormalizedCrossCorrelation2d
...
from utils.registration_bone_mask import PoseRegressor
from utils.registration_bone import SparseRegistration, VectorizedNormalizedCrossCorrelation2d
...
class Registration:
    def __init__(
        ...
        # self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 64, 32], [0.3, 0.3, 0.4], device=self.device, step=[None, 64 // 2, 32 // 2])
        self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.1, 0.45, 0.45], device=self.device, step=[None, 20, 4])
        # self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 16], [0.2, 0.8], device=self.device, step=[None, 4])
        ...
        self.criterion_add = MultiscaleNormalizedCrossCorrelation2dAdd(device=self.device)
        ...
        # unet_ckpt = torch.load("./checkpoints/deal_noise.ckpt", map_location=device)
        # self.unet.load_state_dict(unet_ckpt["model_state_dict"])


    def initialize_registration(self, img):
        with torch.no_grad():
            offset, mask_loss = self.model(img)
            features = None
    ...
    def run(self, idx):
        ...
        # for i in range(20):
        #     t = 0.5 + i * 0.1
        #     gt_img = torch.pow(toZeroOne(img_ori), t)
        #     tmp = toZeroOne(gt_img) * total_mask
        #     mu = tmp.sum() / total_mask.sum()
        #     tmp[tmp < mu] = 0
        #     plt.figure()
        ...
        img = img_input
        # img_save = (toZeroOne(img.detach().squeeze().cpu().numpy()) * 255).astype(np.uint8)
        # cv2.imwrite("img.jpg", img_save)
        # img = torch.tensor(cv2.imread('img.png', cv2.IMREAD_GRAYSCALE)).unsqueeze(0).unsqueeze(0)
        # img = torch.pow(toZeroOne(img), 1.5)
        # img = self.transforms(img, reverse=False).to(self.device).to(torch.float32)
        ...
        optimizer1, scheduler1 = self.initialize_optimizer(registration, r_lr=15e-3, t_lr=10)
        optimizer2, scheduler2 = self.initialize_optimizer(registration, r_lr=15e-3, t_lr=7.5)
        ...
        wei = torch.ones_like(img, requires_grad=False)
        wei[toZeroOne(img) > 0.8] = 0
        # total_mask = (total_mask.bool() & wei.bool()).float()
        plt.figure()
        plt.imshow(total_mask.cpu().squeeze(0).permute(1, 2, 0), cmap='gray', vmin=0, vmax=1)
        plt.show()
        ...
        tmp = toZeroOne(img) * total_mask
        mu = tmp.sum() / total_mask.sum()

        max_dist = torch.max(torch.abs(tmp - mu))
        linear_weights = 1 - torch.abs(tmp - mu) / max_dist
        # linear_weights = torch.pow(linear_weights, 2)
        plt.figure()
        plt.imshow(linear_weights.cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        plt.show()
        print(linear_weights.min())
        print(linear_weights.max())

        tmp[tmp < mu] = 0
        plt.figure()
        plt.imshow(tmp.cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        plt.show()
        # for i in range(51):
        #     t = 1 + i * 0.1
        #     gt_img = self.drr(None, None, None, pose=gt_pose, bone_attenuation_multiplier=t)
        #     gt_img = self.transforms(gt_img).to(self.device).to(torch.float32)
        #     tmp = toZeroOne(gt_img) * total_mask
        #     mu = tmp.sum() / total_mask.sum()
        #     tmp[tmp < mu] = 0
        ...
        # gaussian_weights = torch.exp(-(img - mu) ** 2 / 2)
        # plt.figure()
        # plt.imshow(gaussian_weights.cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        # plt.show()
        # print(gaussian_weights.min())
        # print(gaussian_weights.max())
        for i in itr:
            ...
            # loss = ncc
            wei1 = torch.tensor(linear_weights, requires_grad=False)
            temp = toZeroOne(pred_img) * total_mask
            mu = temp.sum() / total_mask.sum()
            max_dist = torch.max(torch.abs(temp - mu))
            wei2 = 1 - torch.abs(temp - mu) / max_dist
            # wei2 = torch.pow(wei2, 2)
            # plt.figure()
            # plt.imshow(wei2.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
            # plt.show()
            # wei = torch.tensor(pred_img_bone, requires_grad=False)
            # wei = torch.tensor(bone_mask_gt, requires_grad=False)
            # wei = torch.tensor((pred_img_bone.bool() | bone_mask_gt.bool()).float(), requires_grad=False)
            # wei1 = torch.ones_like(img, requires_grad=False)
            wei2 = torch.ones_like(img, requires_grad=False)
            # wei1[bone_mask_gt > 0.1] = 3
            wei2[pred_img_bone > 0.1] = 3
            # wei[wei <= 0.1] = 1
            # wei[toZeroOne(img) > 0.8] = 0.3
            # plt.figure()
            # plt.imshow(wei.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
            # plt.show()
            if i < self.n_iters / 2:
            # if i < 150:
                # loss = self.criterion(pred_img * pred_img_bone, img * bone_mask_gt)
                loss = self.criterion(pred_img, img, wei2, None)
                # loss = gradient_ncc(pred_img, img, total_mask)
                # loss = self.criterion(1 - toZeroOne(pred_img), 1 - toZeroOne(img))
                # loss2 = self.criterion2(pred_img, img)
                # loss = self.criterion_add(pred_img, img, diff)
                # loss = self.criterion2_ori(1 - toZeroOne(pred_img), 1 - toZeroOne(img), wei)
                # loss = self.criterion(torch.log(1 - toZeroOne(pred_img) + 1e-5), torch.log(1 - toZeroOne(img) + 1e-5), wei)
                # loss = nn.functional.mse_loss(toZeroOne(pred_img) * pred_img_bone, toZeroOne(img) * bone_mask_gt)
                # plt.figure()
                # plt.imshow((toZeroOne(pred_img) * pred_img_bone).detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                # plt.show()
                # plt.figure()
                # plt.imshow((toZeroOne(img) * bone_mask_gt).detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                # plt.show()
                ...
                if i == self.n_iters / 2:
                    l = self.criterion(pred_img, img, wei2, wei1)
                    ncc = self.criterion(gt_img, img, wei2, wei1)
                    l_pred = self.criterion(torch.log(1 - toZeroOne(pred_img) + 1e-5), torch.log(1 - toZeroOne(img) + 1e-5))
                    ncc_gt = self.criterion(torch.log(1 - toZeroOne(gt_img) + 1e-5), torch.log(1 - toZeroOne(img) + 1e-5))
                    ll = self.criterion(pred_img, img, wei)
                    nccc = self.criterion(gt_img, img, wei)
                    l_grad = gradient_ncc(pred_img, img, total_mask)
                    ncc_grad = gradient_ncc(gt_img, img, total_mask)

                    # test = torch.zeros_like(img, device=img.device, dtype=img.dtype)
                    # test[toZeroOne(img) > 0.8] = 1
                    # wei[toZeroOne(img) > 0.8] = 0
                    # plt.figure()
                    # plt.imshow(test.cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                    # plt.show()
                    # testWeiNCC(toZeroOne(pred_img), toZeroOne(img), total_mask, wei, test)
                    print(f"loss={l.item()}, gt_loss={ncc.item()}")

                    tmp = toZeroOne(pred_img) * total_mask
                    mu = tmp.sum() / total_mask.sum()
                    tmp[tmp < mu] = 0
                    plt.figure()
                    plt.imshow(tmp.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                ...
                # loss = self.criterion2(torch.log(1-toZeroOne(pred_img) + 1e-5), torch.log(1-toZeroOne(img) + 1e-5))
                # loss = self.criterion2_add(pred_img, img, diff)
                # loss = self.criterion2(toZeroOne(pred_img), toZeroOne(img))
                # loss = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * total_mask, toZeroOne(img) * total_mask, window_size=11, reduction='mean')
                # loss = self.mIOU(pred_img_bone, bone_mask_gt)
            ...
            tre2 = torch.sqrt(((true_fiducials - pred_fiducials) ** 2).sum(dim=2))
            t2 = tre2.mean()
            tre = torch.mean(tre)
        ...
        times.append(0)
        ssims.append((1 - 2 * kornia.losses.ssim_loss(toZeroOne(img) * total_mask, toZeroOne(gt_img) * total_mask, window_size=11, reduction='mean')).item())
        rot1.append(0)
        rot2.append(0)
        rot3.append(0)
        trans1.append(0)
        ...
        plt.title(f"no.{idx}")
        plt.axis('off')
        plt.savefig(f"reg_result/stage/bone_mask/zyl_stage_xray{idx:03d}.jpg")
        plt.show()
        pred_img_bone = self.transforms(pred_img_bone, reverse=True)
        pred_img_bone = (pred_img_bone - pred_img_bone.min()) / (pred_img_bone.max() - pred_img_bone.min())
...
def main(id_number, parameterization):
    ...
    ckpt = torch.load(f"checkpoints/zyl_800_norm_bone_mask2_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/zyl_800_norm_bone_mask2_noaug_best.ckpt", map_location="cuda:1")
    # ckpt = torch.load(f"checkpoints/zyl_800_norm_bone_mask2_noaug_best.ckpt", map_location="cuda:1")
    ...
    for idx in tqdm(range(29, len(specimen)), ncols=100):
        ...
        df.to_csv(
            f"runs/stage/bone_mask/zyl_stage_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
