...
from ours.utils.grad_similar import gradient_ncc
...
class Registration:
    def __init__(
        ...
        self.criterion2_ori = MultiscaleNormalizedCrossCorrelation2dOri([None, 40, 8], [0.1, 0.45, 0.45], step=[None, 20, 4])
        ...
        self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 40, 8], [0.1, 0.45, 0.45], device=self.device, step=[None, 20, 4])
        ...
        self.style_change =  StyleChanger("/home/zsr/project/contrastive-unpaired-translation/checkpoints/drr_style_solid_5_new3/90_net_G.pth",
    ...
    def run(self, idx):
        ...
        # img = (toZeroOne(img) * 255).astype(np.uint8)
        # img = torch.tensor(cv2.equalizeHist(img)).to(self.device).to(torch.float32).unsqueeze(0).unsqueeze(0)
        ...
        optimizer2, scheduler2 = self.initialize_optimizer(registration, r_lr=15e-3, t_lr=7.5e0)
        ...
        for i in itr:
            ...
            if i < self.n_iters / 2:
            # if i < 150:
            #     loss = self.criterion_ori(1-toZeroOne(pred_img) * pred_img_bone, 1-toZeroOne(img) * bone_mask_gt)
            #     loss = self.criterion(pred_img, img)
                # loss = self.criterion(1 - toZeroOne(pred_img), 1 - toZeroOne(img), wei)
                # loss = self.criterion2(1-toZeroOne(pred_img) * (1 - black), 1-toZeroOne(img) * (1 - black), wei)
                # loss = self.criterion_ori(1-toZeroOne(pred_img), 1-toZeroOne(img), wei)
                # loss = self.criterion2(torch.log(1-toZeroOne(pred_img) + 1e-5), torch.log(1-toZeroOne(img) + 1e-5))
                # loss = self.criterion_add(pred_img, img)
                loss = 0.5 * gradient_ncc(pred_img, img, total_mask) + 0.5 * self.criterion(pred_img, img)

                # plt.figure()
                # plt.imshow(torch.log(1-toZeroOne(pred_img) + 1e-5).detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                # plt.show()
                # plt.figure()
                # plt.imshow(torch.log(1-toZeroOne(img) + 1e-5).detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
                # plt.show()
                # loss = 1 - 2 * kornia.losses.ssim_loss(toZeroOne(gt_img) * total_mask, toZeroOne(img) * total_mask, window_size=11, reduction='mean')
                # loss = self.mIOU(pred_img_bone, bone_mask_gt)
            else:
                if i == self.n_iters / 2:
                    l = self.criterion(pred_img, img)
                    ncc = self.criterion(gt_img, img)
                    l1 = self.criterion(torch.log(1-toZeroOne(pred_img) + 1e-5), torch.log(1-toZeroOne(img) + 1e-5), wei)
                    l1_gt = self.criterion(torch.log(1-toZeroOne(gt_img) + 1e-5), torch.log(1-toZeroOne(img) + 1e-5), wei)
                    ll = self.criterion2(1-toZeroOne(pred_img), 1-toZeroOne(img), wei)
                    nccc = self.criterion2(1-toZeroOne(gt_img), 1-toZeroOne(img), wei)
                    print(f"loss={l.item()}, gt_loss={ncc.item()}")
                # loss = self.criterion2(pred_img, img)
                # loss = self.criterion2(1-toZeroOne(pred_img), 1-toZeroOne(img), wei)
                loss = self.criterion2(torch.log(1 - toZeroOne(pred_img) + 1e-5), torch.log(1 - toZeroOne(img) + 1e-5))
                ...
                # loss = self.mIOU(pred_img_bone, bone_mask_gt)
                # loss = 0.5 * self.criterion2(pred_img, img) + 0.5 * (1 - 2 * kornia.losses.ssim_loss(toZeroOne(pred_img) * total_mask, toZeroOne(img) * total_mask, window_size=11, reduction='mean'))

            img_save = pred_img.detach().clone()
            img_save = img_save.cpu().squeeze(0).squeeze(0)
            img_save = np.array(((img_save - img_save.min()) / (img_save.max() - img_save.min())) * 255).astype(
                np.uint8)
            if i == 0:
                first_img = pred_img
                plt.figure()
        ...
        times.append(0)
        ssims.append((1 - 2 * kornia.losses.ssim_loss(toZeroOne(gt_img) * total_mask, toZeroOne(img) * total_mask, window_size=11, reduction='mean')).item())
        rot1.append(0)
        rot2.append(0)
        rot3.append(0)
        trans1.append(0)
...
def main(id_number, parameterization):
    ...
    nccs = []
    for idx in tqdm(range(50, len(specimen)), ncols=100):
