...
class Registration:
    def __init__(
        ...
        # self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.2, 0.4, 0.4], device=self.device, step=[None, 1, 1])
    ...
    def run(self, idx):
        ...
        optimizer1, scheduler1 = self.initialize_optimizer(registration, r_lr=15e-3, t_lr=3)
        optimizer2, scheduler2 = self.initialize_optimizer(registration, r_lr=12e-3, t_lr=6)
        ...
        for i in itr:
            ...
            # s = torch.tensor([[500.0, 0.0, 0.0]]).to(self.device)
            # t_center = torch.tensor([[-500.0, 0.0, 0.0]]).to(self.device)
            # s = gt_pose.transform_points(s)
            # t_center = gt_pose.transform_points(t_center)
            # result = simple_projection(self.specimen.fiducials[0].detach(), s[0].detach().cpu(), t_center[0].detach().cpu())
...
def main(id_number, parameterization):
    ...
    # ckpt = torch.load(f"checkpoints/zyl_epoch200.ckpt", map_location="cuda:1")
    ...
    nccs = []
    for idx in tqdm(range(19, len(specimen)), ncols=100):
