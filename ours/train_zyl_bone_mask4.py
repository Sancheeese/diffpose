...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            #     plt.show()
            # for im in img_bone.bool().float():
            #     plt.figure()
            #     plt.imshow(im.cpu().permute(1, 2, 0), cmap='gray')
            #     plt.show()
            # img_bone = (img_bone.bool() | diff.bool()).float()
            diff = toZeroOne(diff)
            diff[diff > 0.1] = 1
            diff[diff <= 0.1] = 0
            temp = []
            if not torch.isnan(diff[0][0][0][0]):
                if random.random() < 0.6:
                    for im, d in zip(img_input, diff):
                        temp.append(inpaint_with_opencv(im.unsqueeze(0), d.unsqueeze(0) * 255))
                    img_input = torch.cat(temp)
            # for im in img_bone:
            #     plt.figure()
            #     plt.imshow(im.cpu().permute(1, 2, 0), cmap='gray')
            #     plt.show()
            ...
            pred_offset, mask_loss = model(img_input, img_bone)
            ...
            loss = 1 - ncc + 1e-2 * (log_geodesic + double_geodesic) + 10 * mask_loss
            if loss.isnan().any():
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_800_norm_bone_mask5_best.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_800_norm_bone_mask5_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=1000, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_800_norm_bone_mask5_best.ckpt")
            break
