...
from utils.mask_data import MaskDataAugmentation
...
def train(
    ...
    MaskDataAugmentation("/home/zsr/project/diffpose/ours/tube")
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            img_bone_mask = torch.tanh(50 * img_bone)

            img = transforms(img).to(torch.float32)
            img_input = toZeroOne(img) * img_bone_mask
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_800_norm_bone_mask_best.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_800_norm_bone_mask_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_800_norm_bone_mask_best.ckpt")
            break
