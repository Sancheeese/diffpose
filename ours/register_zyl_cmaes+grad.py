...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 8], [0.2, 0.8], device=self.device, step=[None, 4])        # self.criterion = MultiscaleNormalizedCrossCorrelation2d(device=self.device)
    ...
    def run(self, idx):
        ...
        # r = torch.tensor([[ 1.9393, -0.5446, -0.6034]], device='cuda:2')
        # t = torch.tensor([[288.5745,  27.7836,  80.1263]], device='cuda:2')
        # r = torch.tensor([[1.9446, -0.5950, -0.6285]], device='cuda:2')
        # t = torch.tensor([[280.0512, 24.0358, 79.0513]], device='cuda:2')
        r = x[:3].unsqueeze(0)
        t = x[3:].unsqueeze(0)
        pose = RigidTransform(r, t, parameterization="so3_log_map", device=self.device)

        rot_data = pose.get_rotation(parameterization="so3_log_map")
        xyz_data = pose.get_translation()
