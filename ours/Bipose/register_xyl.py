...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/xyl/img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        plt.axis('off')
        plt.savefig(f"./ret/xyl/xyl_bi_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/xyl/xyl_bi_overlay_{idx}.png", alpha=1)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/xyl_800_bi_best.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(62, 66), ncols=100):
        ...
        df.to_csv(
            # f"runs/grad/orinodicexyl_xray{idx:03d}_{parameterization}.csv",
            f"runs/xyl/xyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
