...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/zyl/img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        plt.axis('off')
        plt.savefig(f"./ret/zyl/pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay = overlay_grayscale_with_red_tensor_save2(img_ori, edge, f"./ret/zyl/overlay_{idx}.png", alpha=1)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/zyl/zyl_bi_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        save_err_with_black(pred_img, gt_img, f"./ret/zyl/zyl_bi_err_black_{idx}.png", 119)
        save_err_half(pred_img, gt_img, img_ori, overlay, edge, "./ret/zyl/zyl_bi", idx)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/zyl_800_bi_pa_so4_best.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(62, 66), ncols=100):
        ...
        df.to_csv(
            ...
            f"runs/zyl/zyl_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
