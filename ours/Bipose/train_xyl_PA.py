...
def train(
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            loss = 1 - ncc + 1e-2 * log_geodesic + so4_loss
            if loss.isnan().any():
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/xyl_800_bi_pa_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/xyl_bi_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/xyl_pi_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/xyl_800_bi_pa_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=2, lr=0.005)
    while True:
        try:
            ...
            main(batch_size=4, n_batches_per_epoch=200, n_epochs=800, accumulate_step=2, lr=0.005, restart="checkpoints/xyl_bi_last.ckpt")
            break
