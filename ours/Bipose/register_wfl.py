...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/wfl/wfl_bi_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        save_err_with_black(pred_img, gt_img, f"./ret/wfl/wfl_bi_err_black_{idx}.png", 119)
        save_err_half(pred_img, gt_img, img_ori, overlay, edge, "./ret/wfl/wfl_bi", idx)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/wfl_800_bi_best.ckpt", map_location="cuda:1")
