...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/ysy/img_ori_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        # img = img_ori
        optimizer, scheduler = self.initialize_optimizer(registration, r_lr=5e-3, t_lr=5)
        ...
        plt.axis('off')
        plt.savefig(f"./ret/ysy/ysy_bi_pred_img_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        ...
        overlay_grayscale_with_red_tensor_save(img_ori, edge, f"./ret/ysy/ysy_bi_overlay_{idx}.png", alpha=1)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/ysy_800_bi_best.ckpt", map_location="cuda:1")
    ...
    # for idx in tqdm(range(62, 66), ncols=100):
        ...
        df.to_csv(
            # f"runs/grad/orinodiceysy_xray{idx:03d}_{parameterization}.csv",
            f"runs/ysy/ysy_xray{idx:03d}_{parameterization}.csv",
            index=False,
        )
