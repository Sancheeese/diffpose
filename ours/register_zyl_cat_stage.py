...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 150], [0.3, 0.7], device=self.device)
        self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.1, 0.45, 0.45], device=self.device)
    ...
    def run(self, idx):
        ...
        transforms_aug = torchvision.transforms.Compose([RandomCLAHE(p=1.0, tile_grid_sizes=[8])])
        ...
        optimizer1, scheduler1 = self.initialize_optimizer(registration)
        optimizer2, scheduler2 = self.initialize_optimizer(registration)
...
def main(id_number, parameterization):
    ...
    ckpt = torch.load(f"checkpoints/zyl_800_cat_best.ckpt", map_location="cuda:1")
