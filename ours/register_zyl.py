...
from utils.registration_feat import PoseRegressor, SparseRegistration, VectorizedNormalizedCrossCorrelation2d
...
class Registration:
    ...
    def initialize_optimizer(self, registration, r_lr=1.5e-2, t_lr=7.5):
    ...
    def run(self, idx):
        ...
        img_cut = (img_change - img_change.min()) / (img_change.max() - img_change.min())
        img_cut[img_change > 0.7] = 1
        plt.figure()
        plt.imshow(img_cut.squeeze(0).cpu().permute(1, 2, 0), cmap='gray')
        plt.show()
