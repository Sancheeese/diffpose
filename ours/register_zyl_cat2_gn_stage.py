...
from utils.registration_unet_cat2_gn import PoseRegressor
...
class Registration:
    def __init__(
        ...
        self.criterion = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.2, 0.4, 0.4], device=self.device)
        self.criterion2 = MultiscaleNormalizedCrossCorrelation2d([None, 40, 9], [0.2, 0.4, 0.4], device=self.device)
    ...
    def run(self, idx):
        ...
        optimizer1, scheduler1 = self.initialize_optimizer(registration, r_lr=10e-3, t_lr=6.5e0)
        optimizer2, scheduler2 = self.initialize_optimizer(registration, r_lr=10e-3, t_lr=5e0)
...
def main(id_number, parameterization):
    ...
    ckpt = torch.load(f"checkpoints/zyl_800_cat_best2_gn.ckpt", map_location="cuda:1")
