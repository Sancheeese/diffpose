...
from ours.utils.grad_similar import dice_coefficient_with_mask, dice_batch
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            if random.random() < 0.7:
                img_bone, tube = augmenter.augment(img_bone)
                # img_bone = img_bone * tube
            # img = toZeroOne(img) * img_bone
            ...
            dice = dice_batch(pred_img, img_bone, mask)
            ...
            loss = 1 - dice + 1e-2 * (log_geodesic + double_geodesic)
            if loss.isnan().any():
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_bone_only_pa_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/zyl_bone_only_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/zyl_bone_only_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_bone_only_pa_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            main(batch_size=8, n_batches_per_epoch=100, n_epochs=800, accumulate_step=1, lr=0.005, restart="checkpoints/zyl_bone_only_last.ckpt")
            break
