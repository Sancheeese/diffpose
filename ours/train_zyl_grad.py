...
import time
import sys
# 获取当前脚本的绝对路径（style_to_drr.py的路径）
current_dir = os.path.dirname(os.path.abspath(__file__))
# 获取项目根目录（假设networks与cut目录同级）
...
from ours.utils.grad_similar import GradSimilarity
...
def train(
    ...
    grad_similar = GradSimilarity(radius=119, weight=(0.4, 0.6))
    geodesic = GeodesicSE3()
    double = DoubleGeodesic(drr.detector.sdr)
    contrast_distribution = torch.distributions.Uniform(1.0, 8.0)
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            # img_input = style_change(img_input)
            # img_input = transforms(img_input)
            img = transforms(img).to(torch.float32)

            # if random.random() < 0.5:
            #     img_input = etransforms(img).to(torch.float32)
            #     img = transforms(img).to(torch.float32)
            # else:
            #     img = transforms(img).to(torch.float32)
            #     img_input = img
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_tube_no_change_best_grad.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("/home/zsr/project/diffpose/ours/checkpoints/zyl_best.ckpt", map_location=device)["loss"]
