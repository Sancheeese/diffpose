...
class Registration:
    ...
    def nlopt_objective(self, x, grad):
        ...
        # plt.figure()
        # plt.imshow(pred_img.cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
        # plt.show()
        weight = 0.5
    ...
    def run(self, idx):
        ...
        trans_bound = 1000.0
        lb = np.array([-np.pi, -np.pi, -np.pi, -trans_bound, -trans_bound, -trans_bound])  # SE(3)下界
        ub = np.array([np.pi, np.pi, np.pi, trans_bound, trans_bound, trans_bound])

        # 创建NLopt优化器
        opt = nlopt.opt(nlopt.GD_STOGO, dim)
