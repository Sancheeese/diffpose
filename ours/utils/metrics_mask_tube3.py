...
class NormalizedCrossCorrelation2d(torch.nn.Module):
    ...
    def forward(self, x1, x2, weight_map=None):
        if weight_map is None:
            weight_map = torch.ones_like(x1, device=x1.device, dtype=x1.dtype)
        ...
        _, c, h, w = x1.shape
        x1, x2 = self.norm(x1, weight_map), self.norm(x2, weight_map)
        ...
        #     # score = (x1 * x2 * weight_map * self.mask).sum()
    ...
    def norm(self, x, wei):
        ...
        mu = (x * mask).sum(dim=[-2, -1], keepdim=True) / ((mask * wei).sum(dim=[-2, -1], keepdim=True) + self.eps)
