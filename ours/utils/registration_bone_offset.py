...
class SparseRegistration(torch.nn.Module):
    def __init__(
        ...
        self.transforms = Transforms(self.height, radius=119)

    def forward(self, isocenter_pose, back_pose, center_pose, n_patches=None, patch_size=None):
        ...
        img_bone = siddon_raycast(source, target, self.drr_bone.density, self.drr.spacing).to(torch.float32)
        if self.n_patches is None:
            img = self.drr.reshape_transform(img, batch_size=len(self.rotation))
            img_bone = self.drr.reshape_transform(img_bone, batch_size=len(self.rotation))
