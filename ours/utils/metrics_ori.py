...
class NormalizedCrossCorrelation2d(torch.nn.Module):
    ...
    def __init__(self, patch_size=None, eps=1e-5, step=1):
        super().__init__()
    ...
    def forward(self, x1, x2, wei=None):
        if self.patch_size is not None:
            ...
            if wei is not None:
                wei = to_patches(wei, self.patch_size, self.step)
        ...
        if wei is None:
            ...
            score = torch.einsum("b...,b...,b...->b", x1, x2, wei)
        score /= c * h * w
        return score
...
class MultiscaleNormalizedCrossCorrelation2d(torch.nn.Module):
    ...
    def __init__(self, patch_sizes=[None], patch_weights=[1.0], eps=1e-5, step=[1]):
        super().__init__()

        assert len(patch_sizes) == len(patch_weights), "Each scale must have a weight"
        self.nccs = [
            NormalizedCrossCorrelation2d(patch_size, step=s) for patch_size, s in zip(patch_sizes, step)
        ]
