...
class NormalizedCrossCorrelation2d(torch.nn.Module):
    ...
    def forward(self, x1, x2, wei1=None, wei2=None):
        ...
        wei = wei1 * wei2
        mean1 = (x1 * wei).sum(dim=[-2, -1], keepdim=True) / ((wei).sum(dim=[-2, -1], keepdim=True) + self.eps)
        mean2 = (x2 * wei).sum(dim=[-2, -1], keepdim=True) / ((wei).sum(dim=[-2, -1], keepdim=True) + self.eps)

        numerator = ((x1 - mean1) * (x2 - mean2) * wei).sum(dim=[-2, -1])
        denom1 = torch.sqrt((wei * (x1 - mean1) ** 2).sum(dim=[-2, -1]))
        denom2 = torch.sqrt((wei * (x2 - mean2) ** 2).sum(dim=[-2, -1]))
    ...
    def set_mask(self, tube_mask):
        ...
        valid = self.mask.sum(dim=[-2, -1], keepdim=True) <= 50
