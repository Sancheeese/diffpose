...
class DRR(nn.Module):
    ...
    def __init__(
        ...
        self.soft_tissue = (-800 < self.volume) & (self.volume <= 350)
        self.bone = 350 < self.volume
...
def set_bone_attenuation_multiplier(self: DRR, bone_attenuation_multiplier: float):
    ...
    soft_part = self.soft_tissue_mask * self.soft_tissue_min
    ...
    # start = time.time()
    # self.density = (self.volume / 1000) * 0.08 + 0.08
    # self.density = torch.clamp(self.density, min=0.0)
    # self.bone_attenuation_multiplier = bone_attenuation_multiplier
    # print("计算花费了" + str(time.time() - start) + "毫秒")
