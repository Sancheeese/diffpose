...
class UNet(nn.Module):
    ...
    def forward(self, x, gt):
        ...
        output = self.act(self.end(up4))

        conv1_gt = self.conv1(gt)
        conv1_gt = self.conv2(self.down1(conv1_gt))
        conv3_gt = self.conv3(self.down2(conv1_gt))
        conv4_gt = self.conv4(self.down3(conv3_gt))

        # 返回用于指导损失的特征对
        return {
            'output': output,
            'guidance_features': {
                'gt_l3_encoder': conv3_gt,  # 第2层编码器特征
                'gt_l4_encoder': conv4_gt,  # 第3层编码器特征
                'l3_decoder': up2,  # 第(L-2)层解码器特征
                'l4_decoder': up1  # 第(L-3)层解码器特征
            }
        }
...
class PoseRegressor(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fuse1 = FusionModule(64, 256, 64)
        self.fuse2 = FusionModule(128, 512, 128)
        self.fuse3 = FusionModule(256, 1024, 256)
        ...
        output = backbone(torch.randn(1, 2, 256, 256)).shape[-1]
        self.xyz_regression = torch.nn.Linear(output, 3)
        self.rot_regression = torch.nn.Linear(output, n_angular_components)

    def forward(self, x):
        x_resize = center_crop_and_resize_v2(x, 242)
        x = batch_crop_largest_square_from_circle(x_resize)
        global_feats = self.unet(x_resize)

        x = self.conv1(torch.cat([x, global_feats['layer1']], dim=1))
        x = self.bn1(x)
        x = self.act1(x)
        x = self.maxpool(x)

        x = self.layer1(x)
        x = self.fuse1(x, global_feats['layer2'])

        x = self.layer2(x)
        x = self.fuse2(x, global_feats['layer3'])

        x = self.layer3(x)
        x = self.fuse3(x, global_feats['layer4'])
