...
class PoseRegressor(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target_layers = {
            'layer1': self.backbone1.layer1,
            'layer2': self.backbone1.layer2,
            'layer3': self.backbone1.layer3,
            'layer4': self.backbone1.layer4
        }
        ...
        self.xyz_regression = torch.nn.Sequential(
            nn.Linear(1024, 512),
            nn.ReLU(inplace=True),
            nn.Linear(512, 256),
            nn.ReLU(inplace=True),
        ...
        self.rot_regression = torch.nn.Sequential(
            nn.Linear(1024, 512),
            nn.ReLU(inplace=True),
            nn.Linear(512, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, n_angular_components)
        )
    ...
    def forward(self, x):
        ...
        x_resize = self.backbone1(x_resize)
        x = self.backbone2(x)

        rot = self.rot_regression(torch.cat([x_resize, x], dim=1))
        xyz = self.xyz_regression(torch.cat([x_resize, x], dim=1))
        return convert(
            [rot, xyz],
