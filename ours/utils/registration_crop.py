...
class PoseRegressor(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        output = self.backbone(torch.randn(1, 1, 256, 256)).shape[-1] * 2
        self.xyz_regression = torch.nn.Linear(output, 3)
        self.rot_regression = torch.nn.Linear(output, n_angular_components)

    def forward(self, x):
        x = center_crop_and_resize_v2(x, 242)
        x_crop = batch_crop_largest_square_from_circle(x)

        x = self.backbone(x, dim=1)
        x_crop = self.backbone2(x_crop, dim=1)
        rot = self.rot_regression(torch.cat([x, x_crop], dim=1))
        xyz = self.xyz_regression(torch.cat([x, x_crop], dim=1))
        return convert(
            [rot, xyz],
