...
class GradientNCC2d(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # patch 化
        if self.patch_size is not None:
            g1 = to_patches(g1, self.patch_size, self.step)
            g2 = to_patches(g2, self.patch_size, self.step)

        assert g1.shape == g2.shape

        # 有效 patch
        g1 = g1[:, self.valid_idx, :, :]
        g2 = g2[:, self.valid_idx, :, :]
...
class MultiscaleGradientNCC2d(nn.Module):
    """
    ...
    def __init__(self, patch_sizes=[None], patch_weights=[1.0], eps=1e-5, device=torch.device('cpu'), step=[1]):
        ...
        self.scales = nn.ModuleList([
            GradientNCC2d(patch_size, device=device, step=s)
