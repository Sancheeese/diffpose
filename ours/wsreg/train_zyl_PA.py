...
def train(
    ...
    # 计算相对路径
    augmenter_path = os.path.join(current_dir, "..", "tube")
    ...
    augmenter = MaskDataAugmentation(augmenter_path)
    ...
    for epoch in range(n_epochs + 1):
        ...
        for batch_idx in (itr := tqdm(range(n_batches_per_epoch), leave=False)):
            ...
            pred_img_bone = drr_bone(None, None, None, pose=pred_pose)
            pred_img_bone = torch.tensor(pred_img_bone).to(torch.float32)
            pred_img_bone = transforms(pred_img_bone, reverse=False)
            ...
            dice = dice_coefficient_with_mask(img_bone, pred_img_bone, None)

            loss = 1 - ncc + 1e-2 * (double_geodesic + log_geodesic) + 1e-1 * (1 - dice)
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/zyl_wsr_best.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/zyl_wsr_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/zyl_wsr_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("checkpoints/zyl_wsr_best.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    main(batch_size=4, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001)
    # main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001)
    while True:
        try:
            # main(batch_size=8, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_wsr_last.ckpt")
            main(batch_size=4, n_batches_per_epoch=100, n_epochs=1000, accumulate_step=1, lr=0.001, restart="checkpoints/zyl_wsr_last.ckpt")
            break
