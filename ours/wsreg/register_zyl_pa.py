...
class Registration:
    ...
    def run(self, idx):
        ...
        plt.axis('off')
        plt.savefig(f"./ret/zyl/zyl_wsr_err_{idx}.png", bbox_inches='tight', pad_inches=0)
        plt.show()
        save_err_with_black(pred_img, gt_img, f"./ret/zyl/zyl_wsr_err_black_{idx}.png", 119)
        save_err_half(pred_img, gt_img, img_ori, overlay, edge, "./ret/zyl/zyl_wsr", idx)
...
def main(id_number, parameterization):
    ckpt = torch.load(f"checkpoints/zyl_wsr_best.ckpt", map_location="cuda:1")
