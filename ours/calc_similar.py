...
def process_csv_file(csv_file, specimen, style_change, drr, drr_bone, height, device="cuda:0"):
    ...
    mpd = torch.mean(mpd)
    true_fiducials_3d, pred_fiducials_3d = specimen.get_3d_fiducials(id_number, pose)
    tre = torch.norm(true_fiducials_3d - pred_fiducials_3d, dim=2)
    tre = torch.mean(tre)

    img, true_pose = specimen[id_number]
    # source1, target1 = make_xrays(true_pose.to(device), drr.detector.source, drr.detector.target)
    # source2, target2 = make_xrays(pose.to(device), drr.detector.source, drr.detector.target)
    # target_err = torch.norm(target1 - target2, dim=2)
    # target_err = torch.mean(target_err)

    # # points_true = true_fiducials_3d[0].detach().cpu().numpy()
    # points_true = target2[0][0:-1:2000, :].detach().cpu().numpy()
    # points_pred = target1[0][0:-1:2000, :].detach().cpu().numpy()
    # x = points_true[:, 0]
    # y = points_true[:, 1]
    # z = points_true[:, 2]
    ...
    # A_pred = np.c_[x_pred, y_pred, np.ones(len(x_pred))]
    # coeff_pred, _, _, _ = np.linalg.lstsq(A_pred, z_pred, rcond=None)
    # a_pred, b_pred, c_pred = coeff_pred[0], coeff_pred[1], coeff_pred[2]
    #
    # # 创建3D图形
    ...
    # #                      edgecolors='w',
    # #                      linewidth=0.5)
    # # scatter = ax.scatter(x_pred, y_pred, z_pred,
    # #                      c='blue',  # 用z值着色
    # #                      cmap='viridis',
    # #                      s=50,  # 点的大小
    # #                      alpha=0.8,
    # #                      edgecolors='w',
    # #                      linewidth=0.5)
    # x_range = np.linspace(x.min() - 10, x.max() + 10, 30)
    # y_range = np.linspace(y.min() - 10, y.max() + 10, 30)
    # X, Y = np.meshgrid(x_range, y_range)
    # Z = a * X + b * Y + c
    # Z_pred = a_pred * X + b_pred * Y + c_pred
    #
    # # 绘制平面
    # plane = ax.plot_surface(X, Y, Z,
    #                         alpha=0.3,
    #                         color='red',
    #                         label='Fitted Plane',
    #                         zorder=5)
    # plane = ax.plot_surface(X, Y, Z_pred,
    #                         alpha=0.3,
    #                         color='blue',
    #                         label='Fitted Plane',
    #                         zorder=5)
    ...
    # if target_err.item() < tre.item():
    #     print("wanle")
    # p = drr(None, None, None, pose=true_pose.to(device))
    # p = transforms(p).to(device)
    # plt.figure()
    # plt.imshow(p.detach().cpu().squeeze(0).permute(1, 2, 0), cmap='gray')
    # plt.show()
...
if __name__ == "__main__":
    ...
    directory_path = f"/home/zsr/project/diffpose/ours/case/xyl/runs/mask" # 替换为你的CSV文件目录
