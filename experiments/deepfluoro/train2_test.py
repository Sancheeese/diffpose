...
def load(id_number, height, device):
    specimen = DeepFluoroDataset(id_number, filename="/media/sda1/PersonalFiles/yx/project/diffpose/data/ipcai_2020_full_res_data.h5")
    ...
    for epoch in range(n_epochs + 1):
        ...
        if losses.mean() < best_loss and not losses.isnan().any():
            ...
            torch.save(
                ...
                f"checkpoints/specimen_{id_number:02d}_best_test.ckpt",
            )
        ...
        torch.save(
            ...
            f"checkpoints/specimen_{id_number:02d}_last.ckpt",
        )
        ...
        if record_epoch % 50 == 0:
            torch.save(
                ...
                f"checkpoints/specimen_{id_number:02d}_epoch{record_epoch:03d}.ckpt",
            )
...
def main(
    ...
    if restart is not None:
        ...
        best_loss = torch.load("/media/sda1/PersonalFiles/yx/project/diffpose/experiments/deepfluoro/checkpoints/specimen_01_best_test.ckpt", map_location=device)["loss"]
...
if __name__ == "__main__":
    ...
    gpu_ids = [1, 2, 3]
    gpu_idx = [gpu_ids[i % len(gpu_ids)] for i in id_numbers]
    ...
    while (True):
        try:
            main(1, batch_size=4, n_batches_per_epoch=200, restart="/media/sda1/PersonalFiles/yx/project/diffpose/experiments/deepfluoro/checkpoints/specimen_01_epoch1000.ckpt")
    ...
    #     print(gpu_idx[i])
    #     future = executor.submit(main, id_number, gpu_idx[i])
